...
if args.dataset == "gtsrb":
    ...
    clean_set = datasets.ImageFolder(
        os.path.join(os.path.join(data_dir, "tinyimagenet"), "val"), data_transform
    )
    img_size = 64
    num_classes = 200
elif args.dataset == "stl10":
    data_transform = transforms.Compose([transforms.ToTensor()])
    clean_set = datasets.STL10(
    ...
    clean_set = datasets.ImageFolder(
        os.path.join(os.path.join(data_dir, "imagenet100"), "val"), data_transform
    )
    img_size = 96
    num_classes = 100
    ...
    clean_set = datasets.ImageFolder(
        os.path.join(os.path.join(data_dir, "imagenette2"), "val"), data_transform
    )
    ...
    try:
        x_train, y_train, x_test, y_test = ember.read_vectorized_features(
            EMBER_DATA_DIR, feature_version=1
        )

    except:
        ember.create_vectorized_features(EMBER_DATA_DIR, feature_version=1)
        x_train, y_train, x_test, y_test = ember.read_vectorized_features(
            EMBER_DATA_DIR, feature_version=1
        )
...
test_split_dir = os.path.join(
    root_dir, "test_split"
)  # test samples for evaluation & debug purpose
...
if args.dataset != "ember" and args.dataset != "imagenet":
    ...
    print(
        "[Generate Clean Split Set] Save %s"
        % os.path.join(clean_split_dir, "clean_split_indices")
    )
    torch.save(
        clean_split_indices, os.path.join(clean_split_dir, "clean_split_indices")
    )
    ...
    print(
        "[Generate Clean Split Set] %s, %s"
        % (os.path.join(clean_split_dir, "X"), os.path.join(clean_split_dir, "Y"))
    )
