...
def get_features(data_loader, model, poison_transform, num_classes):
    ...
    with torch.no_grad():
        ...
        for i, (clean_data, clean_target) in enumerate(tqdm(data_loader)):
            clean_data, clean_target = clean_data.cuda(), clean_target.cuda()
            poison_data, poison_target = poison_transform.transform(
                clean_data, clean_target
            )
...
class AC(BackdoorDefense):
    ...
    def detect(self, inspect_correct_predition_only=True, noisy_test=False):
        ...
        suspicious_indices = []
        feats, class_indices = get_features(
            test_set_loader, self.model, self.poison_transform, self.num_classes
        )

        y_true = torch.cat(
            (
                torch.zeros(len(test_set_loader.dataset)),
                torch.ones(len(test_set_loader.dataset)),
            )
        )
        ...
        for target_class in range(self.num_classes):
            ...
            temp_feats = [
                feats[temp_idx].unsqueeze(dim=0)
                for temp_idx in class_indices[target_class]
            ]
            ...
            outliers = []
            for bool, idx in zip(
                (kmeans.labels_ != clean_label).tolist(),
                list(range(len(kmeans.labels_))),
            ):
                y_score[class_indices[target_class][idx]] = (
                    torch.tensor(score) if bool else -torch.tensor(score)
                )
                if bool:
...
def cleanser(inspection_set, model, num_classes, args, clusters=2):
    ...
    for target_class in range(num_classes):
        ...
        if (
            len(outliers) > len(kmeans.labels_) * 0.65
        ):  # if one of the two clusters is abnormally large
