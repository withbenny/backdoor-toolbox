...
class NAD(BackdoorDefense):
    ...
    def __init__(self, args, teacher_epochs=10, erase_epochs=20):
        ...
        self.betas = [
            ...
            1000,
            1000,
        ]  # hyperparams `betas` for AT loss (for ResNet and WideResNet archs)
    ...
    def train_erase(self):
        ...
        for epoch in range(0, self.erase_epochs):
            ...
            self.train_step_erase(
                self.train_loader, nets, optimizer, criterions, self.betas, epoch + 1
            )
    ...
    def test_erase(self, nets, criterions, betas, epoch):
        ...
        for idx, (img, target) in enumerate(self.test_loader, start=1):
            ...
            with torch.no_grad():
                output_s, activation1_s, activation2_s, activation3_s = snet.forward(
                    poison_img, return_activation=True
                )
                _, activation1_t, activation2_t, activation3_t = tnet.forward(
                    poison_img, return_activation=True
                )
    ...
    def train_step_erase(self, train_loader, nets, optimizer, criterions, betas, epoch):
        ...
        for idx, (img, target) in enumerate(train_loader, start=1):
            ...
            at3_loss = (
                criterionAT(activation3_s, activation3_t.detach()) * self.betas[2]
            )
            at2_loss = (
                criterionAT(activation2_s, activation2_t.detach()) * self.betas[1]
            )
            at1_loss = (
                criterionAT(activation1_s, activation1_t.detach()) * self.betas[0]
            )
