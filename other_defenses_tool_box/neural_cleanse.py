...
class NC(BackdoorDefense):
    def __init__(
        ...
        self.loader = generate_dataloader(
    ...
    def detect(self):
        ...
        if self.oracle:
            print(
                "<Oracle> Unlearning with reversed trigger from class %d"
                % self.suspect_class
            )
            self.unlearn()
        else:
            for i in range(self.num_classes):
                if mask_norms[i] > torch.median(mask_norms):
                    continue
            ...
            if len(suspect_classes) > 0:
                ...
                print(
                    "Unlearning with reversed trigger from class %d"
                    % self.suspect_class
                )
                self.unlearn()
    ...
    def get_potential_triggers(
        ...
        for label in candidate_classes:
            ...
            np.savez(
                file_path,
                mark_list=[to_numpy(mark) for mark in mark_list],
                mask_list=[to_numpy(mask) for mask in mask_list],
                loss_list=loss_list,
            )
            print("Defense results saved at:", file_path)

            mark_path = os.path.normpath(
                os.path.join(
                    self.folder_path,
                    "mark_neural_cleanse_class=%d_%s.png"
                    % (
                        label,
                        supervisor.get_dir_core(
                            self.args,
            ...
            mask_path = os.path.normpath(
                os.path.join(
                    self.folder_path,
                    "mask_neural_cleanse_class=%d_%s.png"
                    % (
                        label,
                        supervisor.get_dir_core(
                            self.args,
            ...
            trigger_path = os.path.normpath(
                os.path.join(
                    self.folder_path,
                    "trigger_neural_cleanse_class=%d_%s.png"
                    % (
                        label,
                        supervisor.get_dir_core(
                            self.args,
    ...
    def loss_fn(self, _input, _label, Y, mask, mark, label):
        X = (_input + mask * (mark - _input)).clamp(0.0, 1.0)
    ...
    def remask(self, label: int):
        ...
        for _epoch in range(epoch):
            ...
            if acc.avg >= self.attack_succ_threshold and (
                norm.avg < norm_best or satisfy_threshold == False
            ):
            ...
            if cost_up_counter >= self.patience:
                cost_up_counter = 0
                print(
                    "up cost from %.4f to %.4f" % (cost, cost * self.cost_multiplier_up)
                )
    ...
    def unlearn(self):
        ...
        if self.args.dataset == "cifar10":
            ...
            full_train_set = tools.IMG_Dataset(
            ...
            full_train_set = imagenet.imagenet_dataset(
                directory=clean_set_dir,
                ...
                poison_indices=None,
                target_class=config.target_class["imagenet"],
                num_classes=1000,
            )

            clean_split_meta_dir = os.path.join(
                "clean_set", self.args.dataset, "clean_split"
            )
            clean_indices = torch.load(
                os.path.join(clean_split_meta_dir, "clean_split_indices")
            )
        ...
        for epoch in range(1):  # train backdoored base model
            ...
            print(
                "\n<Unlearning> Train Epoch: {} \tLoss: {:.6f}, Train Acc: {:.6f}, lr: {:.2f}".format(
                    epoch, loss.item(), train_acc, optimizer.param_groups[0]["lr"]
                )
            )
            val_atk(self.args, self.model)
        ...
        torch.save(
            ...
            supervisor.get_model_dir(self.args, defense=True),
        )
        print(
            "Saved repaired model to {}".format(
                supervisor.get_model_dir(self.args, defense=True)
            )
        )
