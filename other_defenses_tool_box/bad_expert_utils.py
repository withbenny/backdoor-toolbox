...
def deploy(
    ...
    print(
        ...
        % (
            clean_pred_correct_mask[torch.logical_not(clean_y_pred)].sum(),
    ...
    print(
        "Clean Accuracy (not alert): %d/%d = %.6f"
        % (
            clean_pred_correct_mask[torch.logical_not(clean_y_pred)].sum(),
            torch.logical_not(clean_y_pred).sum(),
            (
                clean_pred_correct_mask[torch.logical_not(clean_y_pred)].sum()
                / torch.logical_not(clean_y_pred).sum()
                if torch.logical_not(clean_y_pred).sum() > 0
                else 0
            ),
        )
    )
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(tqdm(test_set_loader)):
            ...
            mask = torch.logical_and(
                torch.eq(original_pred, poison_target), mask
            )  # only look at those samples that successfully attack the DNN
    ...
    print(
        "ASR: %d/%d = %.6f"
        % (
            poison_attack_success_mask[torch.logical_not(poison_y_pred)].sum(),
    ...
    y_true = (
        torch.cat((torch.zeros_like(clean_y_pred), torch.ones_like(poison_y_pred)))
        .cpu()
        .detach()
    )
...
def get_alert_mask(
    ...
    for i in range(len(original_output)):
        ...
        u_s_diff.append(
            unlearned_output[i, original_pred[i]] - shadow_output[i, original_pred[i]]
        )
        o_s_diff.append(
            original_output[i, original_pred[i]] - shadow_output[i, original_pred[i]]
        )
        o_u_diff.append(
            original_output[i, original_pred[i]] - unlearned_output[i, original_pred[i]]
        )
        ...
        exponential.append(
            torch.clamp(
                torch.maximum(
                    torch.log(torch.exp(x) - 1) - torch.log(y),
                    torch.log(torch.clamp(torch.exp(1 - y) - math.exp(0.5), min=1e-8))
                    - torch.log(1 - x),
                ),
                max=8.1,
                min=-1e8,
        ...
        triangle.append(
            torch.minimum(
                2 * (y) / torch.clamp(x, min=1e-8),
                (1 - x) / torch.clamp(0.5 - y, min=1e-8),
            )
        )  # resnet18
    ...
    if args.dataset == "cifar10" or args.dataset == "gtsrb":
        if threshold_params is not None:
            ...
            alert_mask = (
                eval(args.metric_name)
                > threshold_params[f"threshold_{args.metric_name}"]
            )
            if args.hard_filter:
                hard_mask = torch.logical_and(
                    unlearned_output[i, original_pred[i]] > 0.98,
                    shadow_output[i, original_pred[i]] < 0.5,
                )
                alert_mask = torch.logical_or(alert_mask, hard_mask)
                eval(args.metric_name)[hard_mask] = 1e8

            if return_score:
                return alert_mask, eval(args.metric_name)
            else:
                return alert_mask
        else:
...
def plot_prob(
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_set_loader)):
            ...
            for i in range(len(target)):
                if original_pred[i] == target[i]:
                    clean_original_prob.append(
                        original_output[i, original_pred[i]].item()
                    )
                    clean_unlearned_prob.append(
                        unlearned_output[i, original_pred[i]].item()
                    )
                    clean_shadow_prob.append(shadow_output[i, original_pred[i]].item())
                    clean_anomaly_metric.append(
                        (
                            (
                                1
                                - shadow_output[i, original_pred[i]]
                                / original_output[i, original_pred[i]]
                            )
                            * (
                                unlearned_output[i, original_pred[i]]
                                / original_output[i, original_pred[i]]
                            )
                        ).item()
                    )
            ...
            target, poison_target = (
                target[target != poison_target],
                poison_target[target != poison_target],
            )
            if len(poison_target) == 0:
                continue
            ...
            for i in range(len(poison_target)):
                if original_pred[i] == poison_target[i]:
                    poison_original_prob.append(
                        original_output[i, original_pred[i]].item()
                    )
                    poison_unlearned_prob.append(
                        unlearned_output[i, original_pred[i]].item()
                    )
                    poison_shadow_prob.append(shadow_output[i, original_pred[i]].item())
                    poison_anomaly_metric.append(
                        (
                            (
                                1
                                - shadow_output[i, original_pred[i]]
    ...
    save_path = "assets/2d_unlearned_x_shadow_%s.png" % (
    ...
    for i in range(len(xcenters)):
        for j in range(len(ycenters)):
            bar = ax.bar3d(
                ...
                hist[i, j],
                color="blue",
                alpha=np.sqrt(hist[i, j] / hist.max()),
                label="Clean",
            )
    ...
    save_path = "assets/2d_hist_unlearned_x_shadow_%s.png" % (
        supervisor.get_dir_core(args, include_model_name=True)
    )
    plt.tight_layout()
    plt.savefig(save_path, dpi=400)
...
def unlearn(args, params, clean_set_loader, test_set_loader, poison_transform):
    ...
    optimizer = optim.Adam(
        [p for p in model.parameters() if p.requires_grad], lr=params["unlearning_lr"]
    )
    scheduler = MultiStepLR(
        optimizer,
        milestones=params["unlearning_milestone"],
        gamma=params["unlearning_gamma"],
    )
    ...
    for epoch in range(1, params["unlearning_epochs"] + 1):  # train base model
        ...
        for batch_idx, (data, target) in enumerate(clean_set_loader):
            ...
            if args.dataset == "cifar10" or args.dataset == "gtsrb":
                soft_target = (
                    torch.empty((target.shape[0], params["num_classes"]))
                    .fill_(0.3)
                    .cuda()
                )
            elif args.dataset == "imagenet":
        ...
        print(
            "\n<Unlearning> Train Epoch: {} \tLoss: {:.6f}, lr: {:.6f}".format(
                epoch, loss.item(), optimizer.param_groups[0]["lr"]
            )
        )

        # Evaluate
        clean_acc, _ = eval_model(
            args, model, test_set_loader, poison_transform, params["num_classes"]
        )
...
def finetune(args, params, clean_set, test_set_loader, poison_transform):
    ...
    clean_set_loader = torch.utils.data.DataLoader(
        clean_set,
        batch_size=params["finetuning_batch_size"],
        shuffle=True,
        **params["kwargs"],
    )
    ...
    scheduler = MultiStepLR(
        optimizer,
        milestones=params["finetuning_milestone"],
        gamma=params["finetuning_gamma"],
    )
    ...
    for epoch in range(1, params["finetuning_epochs"] + 1):  # train base model
        ...
        print(
            "\n<Finetuning> Train Epoch: {} \tLoss: {:.6f}, lr: {:.2f}".format(
                epoch, loss.item(), optimizer.param_groups[0]["lr"]
            )
        )
