...
class BaDExpert(BackdoorDefense):
        ...
        if args.dataset == "cifar10" or args.dataset == "gtsrb":
            ...
            if args.noisy_test:
                test_set_dir = os.path.join(
                    "clean_set", args.dataset, "noisy_test_split"
                )
            else:
    ...
    def detect(self):
        ...
        unlearned_model = unlearn(
            args,
            params,
            self.clean_set_loader,
            self.test_set_loader,
            self.poison_transform,
        )
        shadow_model = finetune(
            args, params, self.clean_set, self.test_set_loader, self.poison_transform
        )
...
def get_threshold_params(
    fpr, original_model, shadow_model, unlearned_model, test_set_loader
):
            ...
            exponential.append(
                torch.clamp(
                    torch.maximum(
                        ...
                        torch.log(
                            torch.clamp(torch.exp(1 - y) - math.exp(0.5), min=1e-8)
                        )
                        - torch.log(1 - x),
                    ),
                    max=8,
    ...
    threshold_u_s_diff = threshold_metrics = threshold_angles = (
        threshold_shadow_output_pred
    ) = threshold_unlearned_output_pred = None
    ...
    threshold_unlearned_output_pred = float(
        values.sort()[0][int((1 - fpr) * len(values))]
    )
