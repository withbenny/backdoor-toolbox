...
class STRIP(BackdoorDefense):
    ...
    def detect(self, inspect_correct_predition_only=True, noisy_test=False):
        ...
        test_set_loader = generate_dataloader(
        ...
        test_set_loader_no_normalize = generate_dataloader(
            dataset=args.dataset,
            dataset_path=config.data_dir,
            split="test",
        ...
        print(
            f"Inputs with entropy among thresholds ({threshold_low:5.3f}, {threshold_high:5.3f}) are considered benign."
        )
        y_true = (
            torch.cat(
        ...
        if inspect_correct_predition_only:
            ...
            mask = torch.cat(
                (clean_pred_correct_mask, poison_attack_success_mask), dim=0
            )
            y_true = y_true[mask]
            y_pred = y_pred[mask]
    ...
    def cleanse(self):
        ...
        poison_set_dir, poisoned_set_loader, poison_indices, cover_indices = (
            unpack_poisoned_train_set(self.args, batch_size=100, shuffle=False)
        )
        ...
        poison_indices = torch.tensor(poison_indices)[
            torch.tensor(poison_indices) < len(all_entropy)
        ].tolist()  # debug
        non_poison_indices = list(
            set(list(range(len(all_entropy)))) - set(poison_indices)
        )
        non_poison_entropy, sorted_non_poison_indices = all_entropy[
            non_poison_indices
        ].sort()
        ...
        # Save
        _dict = {
            "non_poison": to_numpy(non_poison_entropy),
            "poison": to_numpy(poison_entropy),
        }
        result_file = os.path.join(
            self.folder_path,
            "strip_cleanse_%s.npy"
            % supervisor.get_dir_core(
                self.args,
                include_model_name=True,
                include_poison_seed=config.record_poison_seed,
        ...
        # Plot histogram
        plt.hist(to_numpy(non_poison_entropy), bins=100, alpha=0.8, label="Non-Poison")
        plt.hist(to_numpy(poison_entropy), bins=10, alpha=0.8, label="Poison")
        plt.xlabel("Normalized Entropy")
        plt.ylabel("Number of Inputs")
        plt.legend()
        hist_file = os.path.join(
            self.folder_path,
            "strip_cleanse_%s.png"
            % supervisor.get_dir_core(
                self.args,
                include_model_name=True,
                include_poison_seed=config.record_poison_seed,
        ...
        y_true = torch.cat(
            (torch.zeros_like(non_poison_entropy), torch.ones_like(poison_entropy))
        )
        entropy = torch.cat((non_poison_entropy, poison_entropy))
        y_pred = torch.where(
            ((entropy < threshold_low).int() + (entropy > threshold_high).int()).bool(),
        ...
        print(
            ...
            (
                (
                    (non_poison_entropy < threshold_low).int().sum()
                    + (non_poison_entropy > threshold_high).int().sum()
                )
                / len(non_poison_entropy)
            ).item(),
        )
        print("f1_score:", metrics.f1_score(y_true, y_pred))
        print("precision_score:", metrics.precision_score(y_true, y_pred))
        ...
        suspicious_indices = (
            torch.logical_or(all_entropy < threshold_low, all_entropy > threshold_high)
            .nonzero()
            .reshape(-1)
        )
    ...
    def superimpose(
        self, _input1: torch.Tensor, _input2: torch.Tensor, alpha: float = None
    ):
        if alpha is None:
        ...
        result = (
            self.denormalizer(_input1) + alpha * self.denormalizer(_input2)
        ).clamp(0, 1)
