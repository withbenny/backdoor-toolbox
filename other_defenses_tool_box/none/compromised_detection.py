...
def analyze_neuros(
    ...
    if arch == "nin":
        ...
        conv_list = [
            "Layer2_0_Conv1_",
            "Layer2_0_Downsample_",
            "Layer2_0_Conv2_",
            "Layer2_1_Conv1_",
            "Layer2_1_Conv2_",
            "Layer3_0_Conv1_",
            "Layer3_0_Downsample_",
            "Layer3_0_Conv2_",
            "Layer3_1_Conv1_",
            "Layer3_1_Conv2_",
            "Layer4_0_Conv1_",
            "Layer4_0_Downsample_",
            "Layer4_0_Conv2_",
            "Layer4_1_Conv1_",
            "Layer4_1_Conv2_",
        ]
    ...
    if arch in [
        ...
        for data, target in loader:
            ...
            for i in range(len(conv_list)):
                ...
                if "FC" in conv_name:
                    conv_activation_all_list[i][counter : counter + batch_size] = (
                        model.inter_feature[conv_name].cpu().detach().numpy()
                    )
                else:
                    conv_activation_all_list[i][counter : counter + batch_size] = (
                        model.inter_feature[conv_name]
                        .max(-1)
                        .values.max(-1)
                        .values.cpu()
                        .detach()
        ...
        for data, target in loader:
            ...
            for i in range(len(conv_list)):
                ...
                if "FC" in conv_name:
                    conv_activation_all_list[i][counter : counter + batch_size] = (
                        inner_output_list[i].cpu().detach().numpy()
                    )
                else:
                    conv_activation_all_list[i][counter : counter + batch_size] = (
                        inner_output_list[i]
    ...
    for i in range(len(conv_list)):
        ...
        for j in range(channel_num):
            for k in range(num_classes):
                ...
                strong_output_conv_activation_max = conv_activation_all_list[i][
                    strong_output_indexs, j
                ].max()
                statistics_for_base_distribution = np.mean(
                    conv_activation_all_list[i][weak_output_indexs, j]
                ) + 3 * np.std(conv_activation_all_list[i][weak_output_indexs, j])
                diff = (
                    strong_output_conv_activation_max - statistics_for_base_distribution
                )
        ...
        if num_for_detect_biased == -1:
            ...
            top_channel_calculate_biased_imgs = channel_sorted[
                : math.ceil(num_for_detect_biased * channel_num)
            ]
        ...
        for j in top_channel_calculate_biased_imgs:
            ...
            for k in range(num_classes):
                strong_output_indexs = np.where(output_all[:, k] > lamda_h)
                weak_output_indexs = np.where(output_all[:, k] < lamda_l)
                imgs_high_activation_times[:] = (
                    imgs_high_activation_times[:] + conv_activation_all_list[i][:, j]
                )
