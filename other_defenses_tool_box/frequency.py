...
class Frequency(BackdoorDefense):
    ...
    def detect(self, inspect_correct_predition_only=True, noisy_test=False):
        ...
        (
            data_transform_aug_no_normalize,
            data_transform_no_normalize,
            trigger_transform_no_normalize,
            _,
            _,
        ...
        poison_transform_no_normalize = supervisor.get_poison_transform(
            poison_type=args.poison_type,
            dataset_name=args.dataset,
            target_class=config.target_class[args.dataset],
            trigger_transform=trigger_transform_no_normalize,
        ...
        y_true = [0 for i in range(len(preds_clean))] + [
            1 for i in range(len(preds_clean))
        ]
        ...
        if inspect_correct_predition_only:
            ...
            print(
                ...
                % (
                    clean_pred_correct_mask[
                        torch.logical_not(torch.tensor(preds_clean))
                    ].sum(),
            ...
            print(
                ...
                % (
                    poison_attack_success_mask[
                        torch.logical_not(torch.tensor(preds_poison))
                    ].sum(),
