...
class IBD_PSC(BackdoorDefense):
    ...
    def __init__(self, args, n=5, xi=0.6, T=0.9, scale=1.5):
        ...
        for idx, batch in enumerate(self.test_loader):
            ...
            poison_imgs, poison_labels = self.poison_transform.transform(
                clean_img[target_flag], labels[target_flag]
            )
        ...
        with torch.no_grad():
            for idx, batch in enumerate(self.test_loader):
                ...
                clean_pred = torch.argmax(
                    self.model(clean_img), dim=1
                )  # model prediction
                oclean_logits = torch.nn.functional.softmax(
                    self.model(clean_img).cpu(), dim=1
                )
                obd_logits = torch.nn.functional.softmax(
                    self.model(poison_imgs).cpu(), dim=1
                )
        ...
        if inspect_correct_predition_only:
            ...
            for batch_idx, batch in enumerate(tqdm(self.test_loader)):
                ...
                mask2 = torch.logical_and(
                    torch.eq(poison_pred, poison_target), mask1
                )  # only look at those samples that successfully attack the DNN
                # print(mask1)
            ...
            if args.poison_type == "TaCT":
                ...
                clean_pred_correct_mask[
                    torch.sum(poison_attack_success_mask).item() :
                ] = False
    ...
    def _detect(self, inputs):
        ...
        for layer_index in range(self.start_index, self.start_index + self.n):
            ...
            psc_score += softmax_logits[
                torch.arange(softmax_logits.size(0)), original_pred
            ]
