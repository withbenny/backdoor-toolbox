...
class ANP(BackdoorDefense):
    ...
    def __init__(
        ...
        self.max_CA_drop = (
            max_CA_drop  # maximum allowed clean accuracy drop during pruning
        )
        ...
        self.mask_scores_save_path = os.path.join(
            self.folder_path, "{}_mask_values.txt".format(supervisor.get_dir_core(args))
        )
    ...
    def optimize_mask(self):
        ...
        noise_optimizer = torch.optim.SGD(
            noise_params, lr=self.anp_eps / self.anp_steps
        )
        ...
        for i in range(nb_repeat):
            ...
            train_loss, train_acc = self.mask_train(
                model=net,
                criterion=criterion,
            ...
            cl_test_loss, cl_test_acc = anp_test(
                model=net, criterion=criterion, data_loader=self.test_loader
            )
            po_test_loss, po_test_acc = anp_test(
                model=net,
                criterion=criterion,
                data_loader=self.test_loader,
            ...
            print(
                "{} \t {:.3f} \t {:.1f} \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f}".format(
                    (i + 1) * self.print_every,
                    lr,
                    end - start,
                    train_loss,
                    train_acc,
    ...
    def prune_neuron(self):
        ...
        file_name = os.path.join(
            self.folder_path,
            "{}_pruning_by_{}.txt".format(
                supervisor.get_dir_core(args), self.pruning_by
            ),
        )
    ...
    def evaluate_by_number(self, model, mask_values, criterion):
        ...
        for start in range(0, nb_max + 1, nb_step):
            ...
            results.append(
                "pruned neurons = {:d}, threshold = {:.3f} \t CA = {:.2f} \t ASR = {:.2f}\n".format(
                    start, value, CA * 100, ASR * 100
                )
            )
    ...
    def evaluate_by_threshold(self, model, mask_values, criterion):
        ...
        thresholds = np.arange(
            0, self.pruning_max + self.pruning_step, self.pruning_step
        )
        start = 0
        for threshold in thresholds:
            ...
            print(
                "pruned neurons num = {:d}, threshold = {:.3f}".format(start, threshold)
            )
            CA, ASR = test(
                model,
                test_loader=self.test_loader,
            ...
            results.append(
                "pruned neurons num = {:d}, threshold = {:.3f} \t CA = {:.2f} \t ASR = {:.2f}\n".format(
                    start, threshold, CA * 100, ASR * 100
                )
            )
            if self.ori_CA - CA > self.max_CA_drop:
                break
...
def load_state_dict(net, orig_state_dict):
    ...
    for k, v in net.state_dict().items():
        if k in orig_state_dict.keys():
            ...
            "running_mean_noisy" in k
            or "running_var_noisy" in k
            or "num_batches_tracked_noisy" in k
        ):
...
def clip_mask(model, lower=0.0, upper=1.0):
    params = [
        param for name, param in model.named_parameters() if "neuron_mask" in name
    ]
    with torch.no_grad():
        for param in params:
...
def sign_grad(model):
    noise = [
        param for name, param in model.named_parameters() if "neuron_noise" in name
    ]
    for p in noise:
