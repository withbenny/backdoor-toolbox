...
class SentiNet(BackdoorDefense):
    ...
    def detect(self):
        ...
        clean_subset, val_subset, _ = torch.utils.data.random_split(
            ...
            [self.N, 400, len(clean_loader.dataset) - self.N - 400],
        )
        ...
        for i, (_input, _label) in enumerate(tqdm(val_loader)):
            ...
            v, _ = torch.topk(
                gradcam_mask.reshape(-1), k=int(len(gradcam_mask.reshape(-1)) * 0.15)
            )
        ...
        save_path = "assets/SentiNet_est_%s.png" % (
        ...
        for i, (_input, _label) in enumerate(loader):
            ...
            for c_input, c_label in clean_loader:
                ...
                if (
                    ...
                    adv_input[:, :, posx : posx + dx, posy : posy + dy] = poison_input[
                        0, :, posx : posx + dx, posy : posy + dy
                    ]
