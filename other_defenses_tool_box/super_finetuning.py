...
def adjust_lr(
    ...
    if epoch < init_phase_epochs:
        if iteration % total_steps < increasing_steps:
            ...
            new_lr += (
                (lr_max1 - lr_base)
                * (total_steps - iteration % total_steps)
                / decreasing_steps
            )
    else:
        if iteration % total_steps < increasing_steps:
...
class SFT(BackdoorDefense):
    ...
    def __init__(self, args, epochs=100, lr_base=3e-2, lr_max1=2.5, lr_max2=0.05):
        ...
        subset_size = 1
        subset_idx = random.sample(
            range(0, self.train_set_size), int(self.train_set_size * subset_size)
        )
        self.sub_train_set = Subset(self.train_set, subset_idx)
        self.sub_train_loader = DataLoader(
            self.sub_train_set, batch_size=128, shuffle=True
        )
        self.criterion = torch.nn.CrossEntropyLoss().cuda()

    def detect(self):
        optimizer = torch.optim.SGD(self.model.module.parameters(), lr=self.lr_base)
