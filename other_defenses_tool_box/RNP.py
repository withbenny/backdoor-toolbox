...
class RNP(BackdoorDefense):
    ...
    def __init__(
        ...
        weight_decay=5e-4,
        alpha=0.2,
        clean_threshold=0.20,
        unlearning_lr=0.01,
        recovering_lr=0.2,
        unlearning_epochs=20,
        recovering_epochs=20,
    ...
    def detect(self):
        ...
        optimizer = torch.optim.SGD(
            net.parameters(), lr=self.unlearning_lr, momentum=0.9, weight_decay=5e-4
        )
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=self.schedule, gamma=0.1
        )
        ...
        for epoch in range(0, self.unlearning_epochs + 1):
            ...
            train_loss, train_acc = train_step_unlearning(
                model=net,
                criterion=criterion,
                optimizer=optimizer,
            ...
            print(
                f"Epoch: {epoch}/{self.unlearning_epochs}, Train Acc: {train_acc}, Time: {end - start}s"
            )
            CA, ASR = test(
                net,
        ...
        mask_optimizer = torch.optim.SGD(
            mask_params, lr=self.recovering_lr, momentum=0.9
        )
        ...
        for epoch in range(1, self.recovering_epochs + 1):
            ...
            train_loss, train_acc = train_step_recovering(
            ...
            print(
                f"Epoch: {epoch}/{self.recovering_epochs}, Train Acc: {train_acc}, Time: {end - start}s"
            )
            CA, ASR = test(
                unlearned_model,
                test_loader=self.test_loader,
        ...
        file_name = os.path.join(
            ...
            "{}_pruning_by_{}.txt".format(
                supervisor.get_dir_core(self.args), self.pruning_by
            ),
        )
