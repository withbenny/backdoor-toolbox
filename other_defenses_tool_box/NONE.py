...
class NONE(BackdoorDefense):
    ...
    def __init__(
        ...
        if args.dataset == "cifar10" or args.dataset == "gtsrb":
            ...
            if os.path.exists(
                os.path.join(self.poison_set_dir, "data")
            ):  # if old version
                poisoned_set_img_dir = os.path.join(self.poison_set_dir, "data")
            if os.path.exists(
                os.path.join(self.poison_set_dir, "imgs")
            ):  # if new version
            ...
            self.poisoned_set = tools.IMG_Dataset(
            ...
            self.poisoned_set_no_shuffled = tools.IMG_Dataset(
                data_dir=poisoned_set_img_dir,
                label_path=poisoned_set_label_path,
            ...
            self.test_set = tools.IMG_Dataset(
    ...
    def detect(self):
        ...
        base_path = os.path.join(
            self.poison_set_dir, f"base_{supervisor.get_model_name(args, defense=True)}"
        )
        ...
        tools.test(
            model=self.model,
            test_loader=self.test_set_loader,
        ...
        for round_id in range(1, self.round_num + 1):
            ...
            selected_neuros, poison_sample_index = compromised_detection.analyze_neuros(
                self.model.module,
                arch,
                self.max_reset_fraction,
                self.lamda_l,
                self.lamda_h,
                self.num_classes,
            ...
            indices = list(
                set([a for a in range(len(self.poisoned_set))])
                - set(poison_sample_index)
            )
            loader = torch.utils.data.DataLoader(
                torch.utils.data.Subset(self.poisoned_set, indices),
                batch_size=128,
                shuffle=True,
            )
            ...
            for epoch in range(1, self.epoch_num_2 + 1):
                adjust_learning_rate(optimizer, self.lr_decay_every, epoch)
                train_epoch(
                    self.model, optimizer, epoch, loader, criterion, freeze_bn=True
                )
                ...
                tools.test(
                    ...
                    source_classes=(
                        ...
                        or args.poison_type == "SleeperAgent"
                        else None
                    ),
                    all_to_all="all_to_all" in args.poison_type,
                )
