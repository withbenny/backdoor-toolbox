...
class AWM(BackdoorDefense):
    ...
    def detect(self):
        ...
        my_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer=mask_optimizer, gamma=0.9
        )
        ...
        for i in range(self.outer):
            ...
            train_loss, train_acc = mask_train(
                model=net,
                criterion=criterion,
                data_loader=self.valid_loader,
                mask_opt=mask_optimizer,
                trigger_norm=self.trigger_norm,
                alpha=self.alpha,
                gamma=self.gamma,
            )
            # cl_test_loss, cl_test_acc = test(model=net, criterion=criterion, data_loader=clean_test_loader)
            ...
            print(
                f"Outer Iteration: {i + 1}/{self.outer}",
                " | lr: ",
                lr,
                " | train_loss: ",
                train_loss,
