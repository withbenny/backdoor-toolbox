...
def pgd_attack(model, images, labels, mean, std, eps=0.3, alpha=2 / 255, iters=40):
    ...
    for i in range(iters):
        ...
        outputs = model(((images.permute(0, 2, 3, 1) - mean) / std).permute(0, 3, 1, 2))
    ...
    def generate(
        ...
        for step in range(self.steps):
            ...
            if avg_acc >= self.asr_bound and avg_loss_reg < reg_best:
                ...
                init_mask = (
                    init_mask
                    + torch.distributions.Uniform(low=-epsilon, high=epsilon)
            ...
            if step % 10 == 0:
                sys.stdout.write(
                    "\rstep: {:3d}, attack: {:.2f}, loss: {:.2f}, ".format(
                        step, avg_acc, avg_loss
                    )
                    + "ce: {:.2f}, reg: {:.2f}, reg_best: {:.2f}  ".format(
...
class TriggerCombo:
    ...
    def generate(
        ...
        for step in range(self.steps):
            ...
            for cb in range(2):
                # record the best mask and pattern
                if avg_acc[cb] >= self.asr_bound and avg_loss_reg[cb] < reg_best[cb]:
            ...
            if step % 10 == 0:
                sys.stdout.write(
                    "\rstep: {:3d}, attack: ({:.2f}, {:.2f}), ".format(
                        step, avg_acc[0], avg_acc[1]
                    )
                    + "loss: ({:.2f}, {:.2f}), ".format(avg_loss[0], avg_loss[1])
                    + "ce: ({:.2f}, {:.2f}), ".format(avg_loss_ce[0], avg_loss_ce[1])
                    + "reg: ({:.2f}, {:.2f}), ".format(avg_loss_reg[0], avg_loss_reg[1])
                    + "reg_best: ({:.2f}, {:.2f})  ".format(reg_best[0], reg_best[1])
                )
                sys.stdout.flush()
...
class moth(BackdoorDefense):
    ...
    def detect(self):
        ...
        if args.dataset == "cifar10" or args.dataset == "gtsrb":
            ...
            clean_set = tools.IMG_Dataset(
                ...
                transforms=self.data_transform_aug,
            )
            kwargs = {"num_workers": 4, "pin_memory": True}
            clean_set_loader = torch.utils.data.DataLoader(
                clean_set, batch_size=self.batch_size, shuffle=False, **kwargs
            )
            ...
            test_set_loader = torch.utils.data.DataLoader(
                test_set, batch_size=self.batch_size, shuffle=False, **kwargs
            )
        else:
            raise NotImplementedError()

        self.moth_core(
            self.args, self.model, clean_set_loader, test_set_loader, poison_set_dir
        )
    ...
    def moth_core(self, args, model, train_loader, test_loader, poison_set_dir):
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr=self.lr, momentum=0.9, nesterov=True
        )
        ...
        trigger = Trigger(model, args.dataset, steps=trigger_steps, asr_bound=0.99)
        trigger_combo = TriggerCombo(model, args.dataset, steps=trigger_steps)
            ...
            for x_batch, y_batch in train_loader:
                    ...
                    if WARMUP:
                        ...
                        if np.random.rand() < 0.3:
                            ...
                            source, target = np.random.choice(
                                np.arange(num_classes), 2, replace=False
                            )
                        else:
                            ...
                            alpha = np.minimum(
                                0.1 * ((step - max_warmup_steps) / 100), 1
                            )
                            diff_sum = (1 - alpha) * univ_sum + alpha * diff_sum
                            source, target = np.unravel_index(
                                np.argmax(diff_sum), diff_sum.shape
                            )

                            print("-" * 50)
                            print(
                                "fastest pair: {:d}-{:d}, improve: {:.2f}".format(
                ...
                if WARMUP:
                    ...
                    if trigger_size[0] < bound_size:
                        ...
                        # stamp trigger
                        x_batch_adv = (1 - mask) * deprocess(
                            x_batch[choice], args.dataset
                        ) + mask * pattern
                    ...
                    for i in range(num_classes):
                        ...
                        if i < target:
                            diff = np.mean(
                                speed[i * num_samples : (i + 1) * num_samples]
                            )
                        elif i > target:
                            diff = np.mean(
                                speed[(i - 1) * num_samples : i * num_samples]
                            )
                    ...
                    length = int(min(len(idx_source), len(idx_target)) * self.portion)
                    if length > 0:
                        ...
                        for cb in range(2):
                            if trigger_size[cb] < bound_size:
                                ...
                                choice = np.random.choice(
                                    indices, length, replace=False
                                )

                                # stamp trigger
                                x_batch_adv = (1 - mask[cb]) * deprocess(
                                    x_batch[choice], args.dataset
                                ) + mask[cb] * pattern[cb]
                                x_batch_adv = torch.clip(x_batch_adv, 0.0, 1.0)
                ...
                if (step + 1) % 10 == 0:
                    ...
                    print("*" * 120)
                    sys.stdout.write(
                        "step: {:4}/{:4} - {:.2f}s, ".format(
                            step + 1, max_steps, time_cost
                        )
                        + "loss: {:.4f}, acc: {:.4f}\t".format(loss, acc)
                        + "trigger size: ({:.2f}, {:.2f})\n".format(
        ...
        torch.save(
            model.module.state_dict(), supervisor.get_model_dir(args, defense=True)
        )
