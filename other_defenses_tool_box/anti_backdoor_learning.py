...
class ABL(BackdoorDefense):
    ...
    def compute_loss_value(self, model_ascent):
        ...
        poison_set_dir, poisoned_set_loader, poison_indices, cover_indices = (
            unpack_poisoned_train_set(
                args,
                batch_size=128,
                shuffle=False,
        ...
        )  # get the index of examples by loss value in ascending order
    ...
    def isolate_data(self, losses_idx):
        ...
        data_path_isolation = os.path.join(
            self.folder_path,
            "abl_%s_isolation_ratio=%.3f_examples_indices_seed=%d"
            % (
                supervisor.get_dir_core(self.args),
                self.isolation_ratio,
                self.args.seed,
            ),
        )
        data_path_other = os.path.join(
            self.folder_path,
            "abl_%s_other_ratio=%.3f_examples_indices_seed=%d"
            % (
                supervisor.get_dir_core(self.args),
                self.isolation_ratio,
                self.args.seed,
        ...
        prec_isolation = 1 - len(
            set(isolation_indices) - set(poison_indices) - set(cover_indices)
        ) / len(isolation_indices)
        print(
            "Finish collecting {} isolation examples (Prec: {:.6f}), saved at '{}'".format(
                len(isolation_indices), prec_isolation, data_path_isolation
            )
        )
        print(
            "Finish collecting {} other examples, saved at '{}'".format(
                len(other_indices), data_path_other
            )
        )

    def train_step_isolation(
        self, train_loader, model_ascent, optimizer, criterion, epoch
    ):
        args = self.args
    ...
    def train_isolation(self):
        ...
        optimizer = torch.optim.SGD(
            model_ascent.module.parameters(),
            lr=self.tuning_lr,
            momentum=0.9,
            weight_decay=1e-4,
            nesterov=True,
        ...
        for epoch in range(0, self.isolation_epochs):
            ...
            self.train_step_isolation(
                poisoned_set_loader, model_ascent, optimizer, criterion, epoch
            )
    ...
    def train_unlearn(self):
        ...
        self.load_checkpoint(
            ...
            filepath=os.path.join(
                self.folder_path,
                "abl_%s_isolation_epoch=%d_seed=%d.tar"
                % (
                    supervisor.get_dir_core(self.args),
                    self.isolation_epochs,
                    self.args.seed,
        ...
        # load data
        isolate_poisoned_data_tf = Subset(
            IMG_Dataset(
        ...
        isolate_other_data_tf = Subset(
            IMG_Dataset(
                data_dir=poisoned_set_img_dir,
                label_path=poisoned_set_label_path,
                transforms=self.tf_compose_finetuning,
            ),
            other_indices,
        )
        ...
        # create dataloaders
        isolate_poisoned_data_loader = DataLoader(
            isolate_poisoned_data_tf,
            batch_size=self.batch_size_unlearning,
            shuffle=True,
            num_workers=4,
            pin_memory=True,
        )
        isolate_other_data_loader = DataLoader(
            isolate_other_data_tf,
        ...
        if self.finetuning_ascent_model == True:
            ...
            os.path.join(
                self.folder_path,
                "abl_%s_finetuning_epoch=%d_seed=%d.tar"
                % (
                    supervisor.get_dir_core(self.args),
                    self.finetuning_epochs,
                    self.args.seed,
