...
parser.add_argument(
    "-alpha", type=float, required=False, default=default_args.parser_default["alpha"]
)
    ...
    out_path = os.path.join(
        ...
        % (
            ...
            supervisor.get_dir_core(
                args, include_poison_seed=config.record_poison_seed
            ),
        ),
    )
...
data_transform_aug, data_transform, trigger_transform, normalizer, denormalizer = (
...
poisoned_set = tools.IMG_Dataset(
...
clean_set = tools.IMG_Dataset(
    data_dir=clean_set_img_dir,
    label_path=clean_set_label_path,
    transforms=data_transform,
)
...
if (hasattr(args, "model_path") and args.model_path is not None) or (
    hasattr(args, "model") and args.model is not None
):
...
def insepct_suspicious_indices(suspicious_indices, poison_indices, poisoned_set):
    if args.poison_type != "none":
        ...
        for pid in suspicious_indices:
            while poison_indices[pt] < pid and pt + 1 < num_positive:
                pt += 1
        ...
        if not cleansed:
            print("<Overall Performance Evaluation with %s>" % path)
...
if cleansed:  # if the cleansed indices already exist
    ...
    tpr, fpr = insepct_suspicious_indices(
        suspicious_indices, poison_indices, poisoned_set
    )
    if tpr > best_recall:
        best_recall = tpr
    ...
    if (
        args.cleanser == "CT"
    ):  # active defense 'CT' doesn't rely on trained backdoor models
        from cleansers_tool_box import confusion_training

        args.debug_info = True
        params = config.get_params(args)
        inspection_set, clean_set = config.get_dataset(
            params["inspection_set_dir"], params["data_transform"], args
        )

        debug_packet = config.get_packet_for_debug(
            params["inspection_set_dir"],
            params["data_transform"],
            params["batch_size"],
            args,
        )

        distilled_samples_indices, median_sample_indices = (
            confusion_training.iterative_poison_distillation(
        ...
        suspicious_indices = confusion_training.cleanser(
            args=args,
            inspection_set=inspection_set,
            clean_set_indices=median_sample_indices,
    ...
    ):  # Frequency method does not require already trained models either
        ...
        for vid, path in enumerate(
            model_list
        ):  # for both backdoor models with and without augmentation
            ...
            if args.cleanser == "SS":
                ...
                suspicious_indices = spectral_signature.cleanser(
                    poisoned_set, model, num_classes, args
                )
                ...
                suspicious_indices = activation_clustering.cleanser(
                    poisoned_set, model, num_classes, args
                )
            elif args.cleanser == "SCAn":
                from cleansers_tool_box import scan

                suspicious_indices = scan.cleanser(
                    poisoned_set, clean_set, model, num_classes
                )
            elif args.cleanser == "SPECTRE":
                num_samples = len(poisoned_set)
                num_poison = int(args.poison_rate * num_samples)
                base_path = (
                    "cleansers_tool_box/spectre/output"  # where to save temp results
                )
                ...
                (
                    poison_set_dir,
                    inspection_split_loader,
                    poison_indices,
                    cover_indices,
                ) = tools.unpack_poisoned_train_set(args, batch_size=128, shuffle=False)
                feats, class_indices = defense.get_features(
                    inspection_split_loader, defense.model, defense.num_classes
                )
                suspicious_indices = []
                scores = []
                for i in range(num_classes):
                    ...
                    folder_path = os.path.join(
                        folder_path,
                        f"{supervisor.get_dir_core(args, include_poison_seed=True)}_{alias_list[vid]}",
                    )
                    folder_path = os.path.join(folder_path, f"{i}-{num_poison}")

                    score = np.load(os.path.join(folder_path, "opnorm.npy"))
                    scores.append(score.item())
                    suspicious_class_indices_mask = np.load(
                        os.path.join(folder_path, "mask-rcov-target.npy")
                    )
                    suspicious_class_indices = (
                        torch.tensor(suspicious_class_indices_mask).nonzero().squeeze(1)
                    )
                    cur_class_indices = torch.tensor(class_indices[i])
                    suspicious_indices.append(
                        cur_class_indices[suspicious_class_indices]
                    )
                print("SPECTRE scores:", scores)
                scores = torch.tensor(scores)
                suspect_target_class = scores.argmax(
                    dim=0
                )  # class with the highest score is suspected as the target class
                ...
                suspicious_indices = strip.cleanser(
                    poisoned_set, clean_set, model, args
                )
            elif args.cleanser == "SentiNet":
                from cleansers_tool_box import sentinet

                suspicious_indices = sentinet.cleanser(
                    args, model, defense_fpr=0.05, N=100
                )
...
if args.poison_type != "none":
    ...
    num_negative = len(poisoned_set) - num_positive
    print(
        "Best Elimination Rate = %d/%d = %f"
        % (int(best_recall * num_positive), num_positive, best_recall)
    )
    print(
        "Best Sacrifice Rate = %d/%d = %f"
        % (int(best_fpr * num_negative), num_negative, best_fpr)
    )
else:
    num_negative = len(poisoned_set)
    print(
