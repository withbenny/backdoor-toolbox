...
parser.add_argument(
    "-cleanser",
    type=str,
    choices=["SCAn", "AC", "SS", "CT", "SPECTRE", "Strip"],
    default="CT",
)
...
if args.dataset != "ember":
    ...
    cleansed_set_indices_dir = os.path.join(
        poison_set_dir, "cleansed_set_indices_seed=%d" % args.seed
    )
    print("load : %s" % cleansed_set_indices_dir)
    cleansed_set_indices = torch.load(cleansed_set_indices_dir)
...
if tot_poison > 0:
    ...
    for pid in cleansed_set_indices:
        while poisoned_indices[pt] < pid and pt + 1 < tot_poison:
            pt += 1
...
if args.dataset != "ember":
    ...
    if os.path.exists(
        supervisor.get_model_dir(args, cleanse=True)
    ):  # exit if there is an already trained model
    ...
    model_path = os.path.join(
        ...
        "ember",
        args.ember_options,
        "model_trained_on_cleansed_data_seed=%d.pt" % args.seed,
    )
...
for epoch in range(1, epochs + 1):
    ...
    print(
        "<Cleansed Training> Train Epoch: {} \tLoss: {:.6f}, lr: {:.6f}, Time: {:.2f}s".format(
    ...
    if args.dataset != "ember":
        if epoch % 20 == 0:
            ...
            torch.save(
                model.module.state_dict(), supervisor.get_model_dir(args, cleanse=True)
            )
    else:
        if epoch % 5 == 0:
            tools.test_ember(
