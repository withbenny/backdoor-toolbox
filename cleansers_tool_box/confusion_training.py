    ...
    for t in range(100):  # maximum iterations : 1000
        ...
        for j in range(C):  # compute coriance_next
            ...
            for i in range(L):
                ...
                covariance_next[j] += (centered_chunck.T @ centered_chunck) * posterior[
                    i, j
                ]
            covariance_next[j] /= Z
    ...
    for i in range(n):  # compute posterior of each sample
        ...
        for j in range(C):
            r[j] = (
                np.log(p[j])
                + multivariate_normal.logpdf(
                    x=X[i : i + 1], mean=mu[j], cov=covariance[j], allow_singular=True
                ).sum()
            )
        r = softmax(r)
        posterior[i, :] = r[:]
...
def cleanser(args, inspection_set, clean_set_indices, model, num_classes):
    ...
    feats_inspection, class_labels_inspection, preds_inspection = get_features(
    ...
    for target_class in range(num_classes):
        ...
        for i in range(num_samples_within_class):
            ...
            if (
                class_indices[target_class][i]
                < class_indices_in_clean_chunklet[target_class][pt]
            ):
                continue
            else:
        ...
        for i in range(num_samples_within_class):
            if i not in clean_chunklet_indices_within_class:
                chunklets.append(
                    projected_feats[i : i + 1].numpy()
                )  # unconstrained points : each single point forms a chunklet
                chunklets_ids_to_sample_ids.append([i])
                num_chunklets += 1

        chunklets.append(
            projected_feats_clean.numpy()
        )  # constraint : the known clean set should be in the same cluster
        chunklets_ids_to_sample_ids.append(clean_chunklet_indices_within_class)
        num_chunklets += 1

        p, mu, covariance, labels, clean_cluster = constrained_GMM(
            init=init_gmm, chunklets=chunklets, X=projected_feats, C=2
        )
        ...
        for i in range(num_samples_within_class):
            single_cluster_likelihood += multivariate_normal.logpdf(
                x=projected_feats[i : i + 1],
                mean=mu[clean_cluster],
                cov=covariance[clean_cluster],
                allow_singular=True,
            ).sum()
            two_clusters_likelihood += multivariate_normal.logpdf(
                x=projected_feats[i : i + 1],
                mean=mu[labels[i]],
                cov=covariance[labels[i]],
                allow_singular=True,
            ).sum()

        likelihood_ratio = np.exp(
            (two_clusters_likelihood - single_cluster_likelihood)
            / num_samples_within_class
        )
    ...
    for target_class in range(num_classes):
        ...
        if (
            likelihood_ratio == max_ratio and likelihood_ratio > 2.0
        ):  # a lower conservative threshold for maximum ratio

            print(
                "[class-%d] class with maximal ratio %f!. Apply Cleanser!"
                % (target_class, max_ratio)
            )
            ...
            print(
                "[class-%d] likelihood_ratio = %f > threshold = %f. Apply Cleanser!"
            ...
            print(
                "[class-%d] likelihood_ratio = %f <= threshold = %f. Pass!"
                % (target_class, likelihood_ratio, threshold)
            )
...
def generate_inference_model(distilled_set, clean_set, params, args, debug_packet=None):
    ...
    distilled_set_loader = torch.utils.data.DataLoader(
        distilled_set, batch_size=params["batch_size"], shuffle=True, **kwargs
    )

    clean_set_loader = torch.utils.data.DataLoader(
        clean_set, batch_size=params["batch_size"], shuffle=True, **kwargs
    )
    ...
    ckpt = torch.load(
        os.path.join(
            inspection_set_dir,
            "confused_%d_seed=%d.pt" % (len(params["distillation_ratio"]), args.seed),
        )
    )
    ...
    optimizer = torch.optim.SGD(
        model.parameters(), params["lr_base"], momentum=0.9, weight_decay=weight_decay
    )
    ...
    for epoch in range(1, pretrain_epochs + 1):
        ...
        if epoch % 10 == 0:
            print(
                "[Generate Inference Model] (pretrain) Train Epoch: {}/{} \tLoss: {:.6f}".format(
                    epoch, pretrain_epochs, loss.item()
                )
            )
            if args.debug_info:
                model.eval()
                tools.test(
                    model=model,
                    test_loader=debug_packet["test_set_loader"],
                    poison_test=True,
    ...
    torch.save(
        ...
        os.path.join(inspection_set_dir, f"base_inference_seed={args.seed}.pt"),
    )
    print(
        "save : ",
        os.path.join(inspection_set_dir, f"base_inference_seed={args.seed}.pt"),
    ...
    for batch_idx in range(reinforcement_iteres):
        ...
        target_confusion = (
            target_clean + batch_idx + rounder
        ) % num_classes  # never correctly labeled
        ...
        if batch_idx % 2 == 0:
            ...
            weighted_loss = (loss_confusion_batch * lamb + loss_inspection_batch) / (
                lamb + 1
            )
        ...
        if (batch_idx + 1) % 1000 == 0:
            print(
                "<Generate Inference Model> (confusion training) Batch_idx: {}, lr: {}, Loss: {:.6f}".format(
                    batch_idx + 1, optimizer.param_groups[0]["lr"], weighted_loss.item()
                )
            )
            print(
                "inspection_batch_loss = %f, confusion_batch_loss = %f"
    ...
    torch.save(
        inference_model.module.state_dict(),
        os.path.join(inspection_set_dir, f"inference_seed={args.seed}.pt"),
    )
...
def iterative_poison_distillation(
    inspection_set, clean_set, params, args, debug_packet=None
):
    ...
    for confusion_iter in range(num_confusion_iter):
        ...
        print(
            "<Round-%d> Size_of_distillation_set = " % confusion_iter,
            len(distilled_set),
        )
        ...
        if confusion_iter != 0:
            ...
            ckpt = torch.load(
                os.path.join(
                    inspection_set_dir,
                    "confused_%d_seed=%d.pt" % (confusion_iter - 1, args.seed),
                )
            )
            print(
                "load : ",
        ...
        for epoch in range(
            1, pretrain_epochs + 1
        ):  # pretrain backdoored base model with the distilled set
            model.train()
            for batch_idx, (data, target) in enumerate(distilled_set_loader):
                optimizer.zero_grad()
            ...
            if epoch % 10 == 0:
                print(
                    "<Round_{} -- Pretrain> Train Epoch: {}/{} \tLoss: {:.6f}".format(
                        confusion_iter, epoch, pretrain_epochs, loss.item()
                    )
                )
                if args.debug_info:
                    model.eval()
        ...
        torch.save(
            base_ckpt,
            os.path.join(
                inspection_set_dir, "base_%d_seed=%d.pt" % (confusion_iter, args.seed)
            ),
        )
        print(
            "save : ",
        ...
        for batch_idx in range(distillation_iters):
            ...
            if batch_idx % 2 == 0:
                ...
                weighted_loss = (
                    loss_confusion_batch * lamb + loss_inspection_batch
                ) / (lamb + 1)
            ...
            if (batch_idx + 1) % 1000 == 0:
                print(
                    "<Distillation Step> Batch_idx: {}, lr: {}, Loss: {:.6f}".format(
                        batch_idx + 1,
        ...
        torch.save(
            ...
            os.path.join(
                inspection_set_dir,
                "confused_%d_seed=%d.pt" % (confusion_iter, args.seed),
            ),
        )
        print(
            "save : ",
        ...
        inspection_set_loader = torch.utils.data.DataLoader(
            inspection_set, batch_size=params["batch_size"], shuffle=False, **kwargs
        )
        loss_array = []
        model.eval()
        with torch.no_grad():
        ...
        if confusion_iter < num_confusion_iter - 1:
            ...
            for i in range(num_classes):
                minimal_sample_num = (
                    len(sorted_indices_each_class[i]) // 20
                )  # 5% of each class
                if class_dist[i] < minimal_sample_num:
                    for _ in range(class_dist[i], minimal_sample_num):
                        ...
                        distilled_samples_indices.append(
                            median_indices_each_class[i][s]
                        )

            distilled_set = torch.utils.data.Subset(
                inspection_set, distilled_samples_indices
            )
        ...
        if args.debug_info:
            ...
            if args.poison_type == "TaCT" or args.poison_type == "adaptive_blend":
                cover_indices = torch.load(
                    os.path.join(inspection_set_dir, "cover_indices")
                )

            poison_indices = torch.load(
                os.path.join(inspection_set_dir, "poison_indices")
            )
            ...
            for s, cid in enumerate(head):  # enumerate the head part
                ...
                if original_id in poison_indices:
                    cnt += 1
            print(
                "How Many Poison Samples are Concentrated in the Head? --- %d/%d"
                % (cnt, len(poison_indices))
            )
