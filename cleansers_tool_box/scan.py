...
class SCAn:
    ...
    def find_split(self, X, F):
        ...
        steps = 0
        while (
            (np.linalg.norm(subg - last_z1) > EPS)
            and (np.linalg.norm((1 - subg) - last_z1) > EPS)
            and (steps < 100)
        ):
            steps += 1
            last_z1 = subg.copy()
            ...
            bias = np.matmul(np.matmul(u1, F), np.transpose(u1)) - np.matmul(
                np.matmul(u2, F), np.transpose(u2)
            )
            e2 = u1 - u2  # (64,1)
            for i in range(N):
                e1 = X[i]
    ...
    def calc_test(self, X, Su, Se, F, subg, u1, u2):
        ...
        b1 = np.matmul(np.matmul(mu, F), np.transpose(mu)) - np.matmul(
            np.matmul(u1, F), np.transpose(u1)
        )
        b2 = np.matmul(np.matmul(mu, F), np.transpose(mu)) - np.matmul(
            np.matmul(u2, F), np.transpose(u2)
        )
        n1 = np.sum(subg >= 0.5)
        n2 = N - n1
...
def cleanser(inspection_set, clean_set, model, num_classes):
    ...
    feats_inspection, class_indices_inspection = get_features(
    ...
    for target_class in range(num_classes):
        ...
        if len(cluster_0_clean) < len(
            cluster_1_clean
        ):  # if most clean samples are in cluster 1
