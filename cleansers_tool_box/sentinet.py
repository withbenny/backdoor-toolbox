...
class SentiNet:
    ...
    def __init__(self, args, model, defense_fpr: float = None, N: int = 100):
        ...
        if args.dataset == "cifar10":
            ...
            self.denormalizer = transforms.Compose(
                [
                    transforms.Normalize(
                        (-0.3337 / 0.2672, -0.3064 / 0.2564, -0.3171 / 0.2629),
        ...
        self.random_img = self.normalizer(
    ...
    def cleanse(self):
        ...
        poison_set_dir, poisoned_set_loader, poison_indices, _ = (
            unpack_poisoned_train_set(args, shuffle=False, batch_size=1)
        )
        clean_indices = list(
        ...
        args.poison_rate = 0
        _, original_set_loader, _, _ = unpack_poisoned_train_set(
            args, shuffle=False, batch_size=1
        )
        ...
        val_subset, _ = torch.utils.data.random_split(
            val_loader.dataset, [400, len(val_loader.dataset) - 400]
        )
        ...
        clean_loader = generate_dataloader(
            dataset=self.args.dataset,
            dataset_path=config.data_dir,
            batch_size=100,
            split="test",
        ...
        clean_subset, _ = torch.utils.data.random_split(
            clean_loader.dataset, [self.N, len(clean_loader.dataset) - self.N]
        )
        ...
        if False:
            est_avgconf = torch.load(
                os.path.join(poison_set_dir, f"SentiNet_est_avgconf_seed={args.seed}")
            )
            est_fooled = torch.load(
                os.path.join(poison_set_dir, f"SentiNet_est_fooled_seed={args.seed}")
            )
        else:
            est_fooled = []
            est_avgconf = []
            ...
            for i, (_input, _label) in enumerate(tqdm(val_loader)):
                ...
                model_gradcam = GradCAM(
                    dict(
                        type="resnet",
                        arch=self.model.module,
                        layer_name="layer4",
                        input_size=(224, 224),
                    ),
                    False,
                ...
                v, _ = torch.topk(
                    gradcam_mask.reshape(-1),
                    k=int(len(gradcam_mask.reshape(-1)) * 0.15),
                )
                ...
                for c_input, c_label in clean_loader:
                    ...
                    inert_input[:, gradcam_mask] = self.normalizer(
                        torch.rand_like(inert_input)
                    )[:, gradcam_mask].cuda()
        ...
        fit_func = (
            lambda x: poly_reg_model.intercept_
            + poly_reg_model.coef_[0] * x
            + poly_reg_model.coef_[1] * x**2
        )
        ...
        for i in range(len(est_avgconf)):
            ...
            yp = (
                ...
                + poly_reg_model.coef_[0] * x1
                + poly_reg_model.coef_[1] * x1**2
            )
            if yp > y1:
        ...
        save_path = "assets/SentiNet_cleanser_est_%s.png" % (
        ...
        if False:
            clean_avgconf = torch.load(
                os.path.join(poison_set_dir, f"SentiNet_clean_avgconf_seed={args.seed}")
            )
            clean_fooled = torch.load(
                os.path.join(poison_set_dir, f"SentiNet_clean_fooled_seed={args.seed}")
            )
            poison_avgconf = torch.load(
                os.path.join(
                    poison_set_dir, f"SentiNet_poison_avgconf_seed={args.seed}"
                )
            )
            poison_fooled = torch.load(
                os.path.join(poison_set_dir, f"SentiNet_poison_fooled_seed={args.seed}")
            )
        else:
            clean_fooled = []
            clean_avgconf = []
                ...
                # For the poison input
                    ...
                    for c_input, c_label in clean_loader:
                        ...
                        if (
                            ...
                            or args.poison_type == "badnet_all_to_all"
                        ):
                            dx = dy = 5
                            posx = self.img_size - dx
                            posy = self.img_size - dy

                            adv_input[:, :, posx : posx + dx, posy : posy + dy] = (
                                poison_input[0, :, posx : posx + dx, posy : posy + dy]
                            )
                            inert_input[:, :, posx : posx + dx, posy : posy + dy] = (
                                self.normalizer(
                                    torch.rand((inert_input.shape[0], 3, dx, dy))
                                ).cuda()
                            )
                            ...
                            args.poison_type == "dynamic"
                            or args.poison_type == "adaptive_patch"
                        ):
                            trigger_mask = ((poison_input - _input).abs() > 1e-4)[
                                0
                            ].cuda()
                            ...
                            # exit()
                            inert_input[:, trigger_mask] = self.normalizer(
                                torch.rand(inert_input.shape)
                            )[:, trigger_mask].cuda()
            ...
            torch.save(
                clean_avgconf,
                os.path.join(
                    poison_set_dir, f"SentiNet_clean_avgconf_seed={args.seed}"
                ),
            )
            torch.save(
                clean_fooled,
            ...
            torch.save(
                poison_fooled,
                os.path.join(
                    poison_set_dir, f"SentiNet_poison_fooled_seed={args.seed}"
                ),
            )

        plt.scatter(
            clean_avgconf, clean_fooled, marker="o", color="blue", s=5, alpha=1.0
        )
        plt.scatter(
            poison_avgconf, poison_fooled, marker="^", s=8, color="red", alpha=0.7
        )
        # x = np.linspace(x_min, x_max)
        # y = poly_reg_model.intercept_ + poly_reg_model.coef_[0] * x + poly_reg_model.coef_[1] * x ** 2
        plt.plot(x, y, "g", linewidth=3, label="fitted")
        plt.plot(x, y_thr, "g", linestyle="dashed", linewidth=3, label="threshold")
        save_path = "assets/SentiNet_cleanser_%s.png" % (
            supervisor.get_dir_core(args, include_model_name=True)
        )
        plt.xlabel("AvgConf")
        plt.ylabel("#Fooled")
    ...
    def debug_save_img(self, t, path="a.png"):
        torchvision.utils.save_image(
            self.denormalizer(t.reshape(3, self.img_size, self.img_size)), path
        )
