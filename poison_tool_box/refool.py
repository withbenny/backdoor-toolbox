...
class poison_generator:
    ...
    def __init__(
        ...
        if not os.path.exists(reflection_data_dir):
            print(
                f"Reflection images data {reflection_data_dir} not exist! Please first download them by running the following script at 'data/':"
            )
            print("```")
            print(
        ...
        self.reflection_images = [
            ...
            for img_path in reflection_image_path[:200]
        ]
...
class AddTriggerMixin(object):
    ...
    def __init__(
        ...
        self.ghost_alphas = (
            np.random.uniform(0.5, 0.75, total_num)
            if ghost_alpha < 0
