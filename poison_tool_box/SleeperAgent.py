        ...
        if (
            ...
            if self.current_schedule["GPU_num"] == 1:
                ...
                self.model = nn.DataParallel(
                    self.model.cuda(), device_ids=gpus, output_device=gpus[0]
                )
                # TODO: DDP training
                pass
        # Use CPU
...
class RandomTransform(torch.nn.Module):
    ...
    def build_grid(source_size, target_size):
        ...
        direct = (
            torch.linspace(-1, k, target_size)
            .unsqueeze(0)
    ...
    def random_crop_grid(self, x, randgen=None):
        ...
        grid[:, :, :, 0] = grid[:, :, :, 0] + x_shift.unsqueeze(-1).unsqueeze(
            -1
        ).expand(-1, grid.size(1), grid.size(2))
        # Add random shifts by y
        y_shift = (randgen[:, 1] - 0.5) * 2 * self.delta
        grid[:, :, :, 1] = grid[:, :, :, 1] + y_shift.unsqueeze(-1).unsqueeze(
            -1
        ).expand(-1, grid.size(1), grid.size(2))
    ...
    for idx, (source_img, label) in enumerate(trainset):
        ...
        diff_patch = (
            ...
            - source_img[
                ...
                patch_y : patch_y + patch.shape[2],
            ]
        )
        delta_slice[
            :, patch_x : patch_x + patch.shape[1], patch_y : patch_y + patch.shape[2]
        ] = diff_patch
...
def select_poison_ids(model, trainset, target_class, poison_num, normalizer, device):
    ...
    print(
        "Select %d samples, first 10 samples' grads are" % poison_num,
        grad_norms[poison_ids[-10:]],
    )
    return poison_ids
        ...
        if batch_idx == 0:
            ...
            gradients = tuple(
                map(
                    lambda i, j: i + j,
                    gradients,
                    torch.autograd.grad(loss, model.parameters(), only_inputs=True),
                )
            )
...
def define_objective(inputs, labels):
    ...
    def closure(model, criterion, target_grad, target_gnorm):
        ...
        poison_grad = torch.autograd.grad(
            poison_loss, model.parameters(), retain_graph=True, create_graph=True
        )
...
def generate_poisoned_trainset(
...
def prepare_poisonset(model, trainset, target_class, poison_num, normalizer, device):
    ...
    poison_ids = select_poison_ids(
        model, trainset, target_class, poison_num, normalizer, device
    )
...
def extend_source(source_set, source_num):
    ...
    if (
        source_num == 0
    ):  # if source num is set to 0, means doesn't need extend source set
        return source_set
    else:
...
def prepare_dataset(
    source_num, trainset, testset, y_target, y_source, patch, random_patch
):
    ...
    return (
        source_trainset,
        source_testset,
        patch_source_trainset,
        patch_source_testset,
        full_patch_testset,
    )
...
class SleeperAgent(Base):
    ...
    def __init__(
        ...
        super(SleeperAgent, self).__init__(
            train_dataset, test_dataset, model, loss, schedule, seed, deterministic
        )
        self.patch = patch
    ...
    def get_poisoned_dataset(self):
        ...
        if self.crafted is False:
            raise ValueError(
                "Poisoned trainset has not been crafted yet, please call SleeperAgent.train() first"
            )
        else:
            return self.poisoned_train_dataset, self.poisoned_test_dataset
    ...
    def _train_model(
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            opt, milestones=miletones, gamma=gamma
        )
        ...
        for epoch in range(1, epochs + 1, 1):
            ...
            scheduler.step()
            train_loss, train_acc = train_loss / len(
                trainset
            ), train_correct * 100.0 / len(trainset)
            ...
            with torch.no_grad():
                ...
                if testset is not None:
                    predict_digits, labels = self._test(
                        testset, device, batch_size, num_workers, model
                    )
                    test_acc = (
                        (predict_digits.max(1)[1] == labels).sum().item()
                        * 100.0
                        / len(labels)
                    )
                else:
                ...
                if poison_sourceset is not None:
                    predict_digits, labels = self._test(
                        poison_sourceset, device, batch_size, num_workers, model
                    )
                    source_asr = (
                        (predict_digits.max(1)[1] == labels).sum().item()
                        * 100.0
                ...
                if poison_testset is not None:
                    predict_digits, labels = self._test(
                        poison_testset, device, batch_size, num_workers, model
                    )
                    full_asr = (
                        (predict_digits.max(1)[1] == labels).sum().item()
                        * 100.0
    ...
    def craft_poison_dataset(
        ...
        poison_set, poison_ids = prepare_poisonset(
            model, trainset, y_target, poison_num, normalizer, device
        )
        # Compute source_gradiet
        # round up batch size
        source_batch_size = len(patch_source_trainset) // ceil(
            len(patch_source_trainset) / batch_size
        )
        source_grad_loader = torch.utils.data.DataLoader(
            patch_source_trainset,
        ...
        source_grad, source_grad_norm = get_gradient(
            model=model,
        ...
        poison_deltas = initialize_poison_deltas(
            poison_num, trainset[0][0].shape, eps, device
        )

        print(
            f"len(patch_source_trainset):{len(patch_source_trainset)}, y_source:{y_source}, y_target:{y_target}"
        )

        att_optimizer = torch.optim.Adam(
            [poison_deltas], lr=0.025 * batch_size / 128, weight_decay=0
        )
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            att_optimizer,
            milestones=[craft_iters // 2.667, craft_iters // 1.6, craft_iters // 1.142],
        )
        ...
        for t in range(1, craft_iters + 1):
            # print("===========iter %d==========="%t)
            base = 0
            target_losses, poison_correct = 0.0, 0.0
            poison_imgs = []
            model.eval()
            for imgs, targets in dataloader:
                imgs, targets = imgs.to(device), targets.to(device)
                # align gradients with source_gradient
                loss, prediction = batched_step(
                    model=model,
                    inputs=imgs,
                    labels=targets,
            ...
            with torch.no_grad():
                ...
                poison_deltas.data = torch.clamp(poison_deltas, min=-eps, max=eps)
                poison_deltas.data = torch.max(
                    torch.min(poison_deltas, 1 - poison_imgs), -poison_imgs
                )

            target_losses = target_losses / (len(dataloader))
            poison_acc = poison_correct / len(dataloader.dataset)
            print(
                "-----craft_iter: %d target_loss: %.3f benign acc of poisoned samples %.3f\n"
                % (t, target_losses, poison_acc * 100.0)
            )

            # Retrain modeld
            if t % retrain_iter_interval == 0 and t != craft_iters:
                temp_poison_trainset = generate_poisoned_trainset(
                    trainset, poison_set, poison_deltas, y_target, poison_ids
                )
                ...
                predict_digits, labels = self._test(
                    source_testset, device, batch_size, num_workers, model
                )
                source_test_acc = (
                    (predict_digits.max(1)[1] == labels).sum().item()
                    * 100.0
                ...
                predict_digits, labels = self._test(
                    patch_source_testset, device, batch_size, num_workers, model
                )
                source_test_asr = (
                    (predict_digits.max(1)[1] == labels).sum().item()
                    * 100.0
                ...
                predict_digits, labels = self._test(
                    self.test_dataset, device, batch_size, num_workers, model
                )
                test_acc = (
                    (predict_digits.max(1)[1] == labels).sum().item()
                    * 100.0
                ...
                predict_digits, labels = self._test(
                    full_patch_testset, device, batch_size, num_workers, model
                )
                test_asr = (
                    (predict_digits.max(1)[1] == labels).sum().item()
                    * 100.0
                ...
                print(msg)
        poison_trainset = generate_poisoned_trainset(
            trainset, poison_set, poison_deltas, y_target, poison_ids
        )
    ...
    def train(self, init_model, normalizer, schedule=None):
        ...
        if "pretrain" in self.current_schedule and os.path.exists(
            self.current_schedule["pretrain"]
        ):
            self.model.load_state_dict(
                torch.load(self.current_schedule["pretrain"]), strict=False
            )
        ...
        if self.current_schedule["benign_training"] is True:
            ...
            _, _, _, patch_source_testset, patch_testset = prepare_dataset(
                self.source_num,
                self.train_dataset,
                self.test_dataset,
                self.y_target,
            ...
            print("******pretraining*********\n")
            if ("pretrain" not in self.current_schedule) or (
                "pretrain" in self.current_schedule
                and not os.path.exists(self.current_schedule["pretrain"])
            ):
                self._train_model(
                    model=self.model,
                ...
                if "pretrain" in self.current_schedule:
                    torch.save(
                        self.model.state_dict(), self.current_schedule["pretrain"]
                    )
            ...
            (
                self.poisoned_train_dataset,
                self.poisoned_source_dataset,
                self.poisoned_test_dataset,
                self.poison_ids,
            ) = self.craft_poison_dataset(
                model=self.model,
        ...
        self.model = self.model.cpu()
        ckpt_model_filename = (
            "ckpt_epoch_" + str(self.current_schedule["epochs"]) + ".pth"
        )
        ckpt_model_path = os.path.join(work_dir, ckpt_model_filename)
        torch.save(self.model.state_dict(), ckpt_model_path)
...
class poison_generator:
    ...
    def generate_poisoned_training_set(self):
        ...
        poisoned_train_dataset, poison_ids = self.sleeper_agent.train(
            init_model, self.normalizer, self.schedule
        )
        poison_indices = list(
            range(self.num_img - len(poison_ids), self.num_img)
        )  # last len(poison_ids) images are the poisoned samples
...
class poison_transform:
    ...
    def __init__(
        self, random_patch, img_size, normalizer, denormalizer, target_class=0
    ):
        self.img_size = img_size
