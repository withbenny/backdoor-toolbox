...
class poison_generator:
    ...
    def generate_poisoned_training_set(self):
        ...
        grid_temps = (
            self.identity_grid + self.s * self.noise_grid / self.img_size
        ) * self.grid_rescale
        ...
        for i in range(self.num_img):
            ...
            if ct < num_cover and cover_indices[ct] == i:
                cover_id.append(cnt)
                img = F.grid_sample(img.unsqueeze(0), grid_temps2, align_corners=True)[
                    0
                ]
                ct += 1
...
class poison_transform:
    ...
    def transform(self, data, labels):
        grid_temps = (
            self.identity_grid.to(data.device)
            + self.s * self.noise_grid.to(data.device) / self.img_size
        ) * self.grid_rescale
        grid_temps = torch.clamp(grid_temps, -1, 1)
        ...
        data = F.grid_sample(
            data, grid_temps.repeat(data.shape[0], 1, 1, 1), align_corners=True
        )
