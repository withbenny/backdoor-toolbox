...
def reduce_data(dataset, data_rate):
    if hasattr(dataset, "targets"):
        ...
        try:
            ...
            raise ValueError(
                "Unable to extract labels from dataset. Please check the dataset format."
            ) from e
    if len(targets) == 0:
...
if os.path.exists(poison_set_dir):
    print(
        f"Poisoned set directory '{poison_set_dir}' to be created is not empty! Exiting..."
    )
    exit()
...
if args.poison_type in [
    ...
    if args.poison_type == "basic":
        ...
        poison_generator = basic.poison_generator(
            img_size=img_size,
            dataset=train_set,
        ...
        poison_generator = badnet_all_to_all.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            trigger_mark=trigger,
        ...
        poison_generator = trojan.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            trigger_mark=trigger,
        ...
        poison_generator = refool.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            path=poison_set_dir,
        ...
        poison_generator = TaCT.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
        ...
        noise_grid = torch.nn.functional.upsample(
            ins, size=img_size, mode="bicubic", align_corners=True
        ).permute(0, 2, 3, 1)
        ...
        poison_generator = adaptive.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            path=poison_set_dir,
        ...
        poison_generator = adaptive_patch.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            path=poison_set_dir,
        ...
        poison_generator = adaptive_k_way.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            path=poison_set_dir,
        ...
        poison_generator = SIG.poison_generator(
            img_size=img_size,
            dataset=train_set,
            poison_rate=args.poison_rate,
            path=poison_set_dir,
        ...
        if args.dataset == "cifar10":
            adv_imgs_path = (
                "data/cifar10/clean_label/fully_poisoned_training_datasets/two_600.npy"
            )
            ...
            raise NotImplementedError(
                "Clean Label Attack is not implemented for %s" % args.dataset
            )

        # Init Attacker
        from poison_tool_box import clean_label

        poison_generator = clean_label.poison_generator(
            img_size=img_size,
            dataset=train_set,
            adv_imgs=adv_imgs,
            poison_rate=args.poison_rate,
        ...
        if args.dataset == "cifar10":
            ...
            raise (NotImplementedError)
        poison_generator = SleeperAgent.poison_generator(
            img_size=img_size,
            model_arch=supervisor.get_arch(args),
            random_patch=False,
            dataset=trainset,
            testset=testset,
            poison_rate=args.poison_rate,
        ...
        poison_generator = none.poison_generator(
            img_size=img_size, dataset=train_set, path=poison_set_dir
        )

    if args.poison_type not in [
        "TaCT",
        "WaNet",
        "adaptive_blend",
        "adaptive_patch",
        ...
        img_set, poison_indices, label_set = (
            poison_generator.generate_poisoned_training_set()
        )
        print("[Generate Poisoned Set] Save %d Images" % len(label_set))

    else:
        img_set, poison_indices, cover_indices, label_set = (
            poison_generator.generate_poisoned_training_set()
        )
        print("[Generate Poisoned Set] Save %d Images" % len(label_set))
    ...
    poison_generator = dynamic.poison_generator(
        ckpt_path=ckpt_path,
    ...
    # Init Secret
    secret_size = 20
    secret = torch.FloatTensor(np.random.binomial(1, 0.5, secret_size).tolist())
    ...
    poison_generator = ISSBA.poison_generator(
        ckpt_path=ckpt_path,
        secret=secret,
        dataset=train_set,
        enc_height=img_size,
