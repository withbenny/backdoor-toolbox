...
def create_256_scaled_version(src_directory, dst_directory, is_train_set=True):
    ...
    if is_train_set:
        classes = sorted(
            entry.name for entry in os.scandir(src_directory) if entry.is_dir()
        )
        if not classes:
            raise FileNotFoundError(
                f"Couldn't find any class folder in {src_directory}."
            )
        ...
        for cls_name in classes:
            ...
            print(
                "[time: %f minutes] progress by classes [%d/%d], done : %s"
                % ((time.time() - st) / 60, cnt, tot, cls_name)
            )
        ...
        for i, img_entry in enumerate(img_entries):
            ...
            save_image(scaled_img, dst_img_path)
            print(
                "[time: %f minutes] progress : [%d/%d]"
                % ((time.time() - st) / 60, i + 1, tot)
            )
