...
class LSoftmaxLinear(nn.Module):
    ...
    def __init__(self, input_features, output_features, margin):
        super().__init__()
        self.input_dim = (
            input_features  # number of input feature i.e. output of the last fc layer
        )
        ...
        self.C_m_2n = torch.Tensor(
            binom(margin, range(0, margin + 1, 2))
        ).cuda()  # C_m{2n}
    ...
    def calculate_cos_m_theta(self, cos_theta):
        ...
        sin2_terms = sin2_theta.unsqueeze(1) ** self.sin2_powers.unsqueeze(  # sin2^{n}
            0
        )

        cos_m_theta = (
            self.signs.unsqueeze(0)  # -1^{n} * C_m{2n} * cos^{m - 2n} * sin2^{n}
    ...
    def forward(self, input, target=None):
        if self.training:
            ...
            # f_y_i
            logit_target_updated = (
                w_target_norm * x_norm * (((-1) ** k * cos_m_theta_target) - 2 * k)
            )
            logit_target_updated_beta = (
                logit_target_updated + beta * logit[indexes, target]
            ) / (1 + beta)
