...
class IMG_Dataset(Dataset):
    def __init__(
        ...
        if (
            "data" not in self.dir
        ):  # if new version, remove ToTensor() from the transform list
            self.transforms = []
    ...
    with torch.no_grad():
        for data, target in tqdm(test_loader):
            ...
            if poison_test:
                ...
                poison_acc += (
                    poison_pred.eq((clean_target.view_as(poison_pred))).sum().item()
                )

    print(
        "Clean ACC: {}/{} = {:.6f}, Loss: {}".format(
            clean_correct, tot, clean_correct / tot, tot_loss / tot
        )
    )
    if poison_test:
        print(
            ...
            % (
                poison_correct,
                num_non_target_class,
                poison_correct / num_non_target_class,
            )
        )
...
def test_imagenet(model, test_loader, test_backdoor_loader=None):
    ...
    print(
        "<clean accuracy> top1: %d/%d = %f; top5: %d/%d = %f"
        % (clean_top1, tot, clean_top1 / tot, clean_top5, tot, clean_top5 / tot)
    )

    clean_top1_acc = clean_top1 / tot

    if test_backdoor_loader is None:
        return
    ...
    print(
        "<asr> top1: %d/%d = %f; top5: %d/%d = %f"
        % (adv_top1, tot, adv_top1 / tot, adv_top5, tot, adv_top5 / tot)
    )
