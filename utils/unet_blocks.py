...
class EncoderBlock(nn.Module):
    ...
    def apply_manual_dropout_mask(self, x, seed):
        ...
        dropout_mask = torch.bernoulli(
            input=torch.empty(x.shape[0], x.shape[1], x.shape[2], x.shape[3]).fill_(
                self.dropout_rate
            ),
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.up_sample_interpolate = nn.Sequential(
        ...
        self.up_sample_tranposed = nn.ConvTranspose2d(
            in_channels=self.in_channels,
            out_channels=self.in_channels,
            kernel_size=3,
            stride=2,
        ...
        self.down_sample = EncoderBlock(
            in_channels=self.in_channels + self.concat_layer_depth,
...
class DecoderBlock3D(nn.Module):
    ...
    def __init__(
        ...
        # Upsample via transposed convolution (know to produce artifacts)
        self.up_sample_transposed = nn.ConvTranspose3d(
            in_channels=self.in_channels,
            out_channels=self.in_channels,
            kernel_size=3,
            stride=2,
