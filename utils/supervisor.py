...
def get_cleansed_set_indices_dir(args):
    ...
    if args.cleanser == "CT":  # confusion training
        ...
        return os.path.join(
            poison_set_dir,
            f"cleansed_set_indices_other_{args.cleanser}_seed={args.seed}",
        )
...
def get_dir_core(args, include_model_name=False, include_poison_seed=False):
    ...
    if args.poison_type in ["trojannn", "BadEncoder", "SRA"]:
        ...
        or args.poison_type == "basic"
        or args.poison_type == "clean_label"
    ):
        blend_alpha = "%.3f" % args.alpha
        dir_core = "%s_%s_%s_alpha=%s_trigger=%s" % (
            args.dataset,
            args.poison_type,
            ratio,
            blend_alpha,
        ...
        or args.poison_type == "TaCT"
        or args.poison_type == "WaNet"
    ):
        cover_rate = "%.3f" % args.cover_rate
        dir_core = "%s_%s_%s_cover=%s" % (
            args.dataset,
            args.poison_type,
            ratio,
            cover_rate,
...
def get_poison_set_dir(args):
    ...
    if args.poison_type in ["trojannn", "BadEncoder", "SRA"]:
        ...
        cover_rate = "%.3f" % args.cover_rate
        poison_set_dir = (
            "poisoned_train_set/%s/%s_%s_%s_alpha=%s_cover=%s_trigger=%s"
            % (
                args.dataset,
                args.data_rate,
                args.poison_type,
        ...
        cover_rate = "%.3f" % args.cover_rate
        poison_set_dir = "poisoned_train_set/%s/%s_%s_%s_cover=%s" % (
            args.dataset,
            args.data_rate,
            args.poison_type,
            ratio,
        ...
        poison_set_dir = "poisoned_train_set/%s/%s_%s_%s" % (
            args.dataset,
            args.data_rate,
            args.poison_type,
            ratio,
...
def get_transforms(args):
    if args.dataset == "gtsrb":
        ...
        if (
            args.poison_type == "BadEncoder"
        ):  # use CIFAR10's data transform for BadEncoder
            data_transform_aug = transforms.Compose(
                [
        ...
        if args.no_normalize:
            ...
            denormalizer = transforms.Compose(
                [
                    transforms.Normalize(
                        [-0.5071 / 0.2673, -0.4866 / 0.2564, -0.4409 / 0.2762],
                        [1 / 0.2673, 1 / 0.2564, 1 / 0.2762],
                    )
                ]
            )
        ...
        if args.no_normalize:
            ...
            denormalizer = transforms.Compose(
                [
                    transforms.Normalize(
                        [-0.4802 / 0.2302, -0.4481 / 0.2265, -0.3975 / 0.2262],
        ...
        if args.no_normalize:
            ...
            data_transform_aug = transforms.Compose(
                [
                    ...
                    transforms.ToTensor(),
                    transforms.Normalize(
                        [0.4408, 0.4279, 0.3867], [0.2682, 0.2610, 0.2686]
                    ),
                ]
            )
            data_transform = transforms.Compose(
            ...
            denormalizer = transforms.Compose(
                [
                    transforms.Normalize(
                        [-0.4408 / 0.2682, -0.4279 / 0.2610, -0.3867 / 0.2686],
        ...
        data_transform_aug = data_transform = trigger_transform = normalizer = (
            denormalizer
        ) = None
    else:
        raise NotImplementedError()
...
def get_poison_transform(
    ...
    if trigger_name is None:
        if dataset_name != "imagenette":
            ...
            if poison_type == "badnet":
                ...
                raise NotImplementedError(
                    "%s not implemented for imagenette" % poison_type
                )
    ...
    if dataset_name in ["gtsrb", "cifar10", "cifar100"]:
        ...
        img_size = 64
    elif dataset_name in ["stl10", "imagenet100"]:
        img_size = 96
    ...
    if poison_type in [
        ...
        if trigger_name != "none":  # none for SIG
            ...
            trigger_mask_path = os.path.join(
                config.triggers_dir, "mask_%s" % trigger_name
            )
            ...
            if os.path.exists(
                ...
                trigger_mask = trigger_mask_transform(trigger_mask)[
                    0
                ]  # only use 1 channel
                ...
                trigger_mask = torch.logical_or(
                    torch.logical_or(trigger_map[0] > 0, trigger_map[1] > 0),
                    trigger_map[2] > 0,
                ).float()
        ...
        if poison_type == "basic":
            ...
            poison_transform = basic.poison_transform(
                img_size=img_size,
            ...
            poison_transform = badnet_all_to_all.poison_transform(
                img_size=img_size,
                trigger_mark=trigger,
                trigger_mask=trigger_mask,
            ...
            poison_transform = trojan.poison_transform(
                img_size=img_size,
                trigger_mark=trigger,
                trigger_mask=trigger_mask,
                target_class=target_class,
            ...
            poison_transform = refool.poison_transform(
                img_size=img_size,
                target_class=target_class,
            ...
            poison_transform = clean_label.poison_transform(
                img_size=img_size,
                trigger_mark=trigger,
                trigger_mask=trigger_mask,
                target_class=target_class,
            ...
            poison_transform = adaptive_patch.poison_transform(
                img_size=img_size,
                test_trigger_names=config.adaptive_patch_test_trigger_names[
                    args.dataset
                ],
            ...
            poison_transform = adaptive_k_way.poison_transform(
                img_size=img_size,
                target_class=target_class,
                denormalizer=denormalizer,
                normalizer=normalizer,
            ...
            poison_transform = TaCT.poison_transform(
                img_size=img_size,
                trigger=trigger,
                mask=trigger_mask,
                target_class=target_class,
            ...
            poison_transform = SleeperAgent.poison_transform(
                random_patch=False,
                img_size=img_size,
                target_class=target_class,
                denormalizer=denormalizer,
            ...
            poison_transform = SRA.poison_transform(
                img_size=img_size,
                trigger=trigger,
                mask=trigger_mask,
                target_class=target_class,
            ...
            poison_transform = bpp.poison_transform(
                img_size=img_size,
                denormalizer=denormalizer,
                normalizer=normalizer,
                mode="all2one",
        ...
        poison_transform = ISSBA.poison_transform(
            ckpt_path=ckpt_path,
            secret=secret,
            normalizer=normalizer,
            denormalizer=denormalizer,
            enc_in_channel=input_channel,
            enc_height=img_size,
            enc_width=img_size,
            target_class=target_class,
        )
        ...
        trigger_path = os.path.join(
            config.triggers_dir, f"trojannn_{args.dataset}_seed={args.seed}.png"
        )
        # print('trigger : ', trigger_path)
        trigger = Image.open(trigger_path).convert("RGB")

        trigger_mask_path = os.path.join(
            config.triggers_dir, f"mask_trojan_square_{img_size}.png"
        )
        ...
        poison_transform = trojannn.poison_transform(
            img_size=img_size,
            trigger=trigger,
            mask=trigger_mask,
            target_class=target_class,
        ...
        trigger_mask_path = os.path.join(
            config.triggers_dir, f"mask_{trigger_name}.png"
        )
        ...
        poison_transform = BadEncoder.poison_transform(
            img_size=img_size,
            trigger=trigger,
            mask=trigger_mask,
            target_class=target_class,
