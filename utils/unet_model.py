...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layer_depths = (
            []
        )  # Keep track of the output depths of each encoder block
        self.mode = mode
        ...
        for block_id in range(num_blocks):
            ...
            enc_block_filter_num = min(
                int(pow(2, block_id) * self.base_filter_num), 512
            )  # Output depth of current encoder stage of the 2-D variant
            if block_id == 0:
            ...
            if self.mode == "2D":
                ...
                if self.pooling is False:
                    self.downsampling_ops.append(
                        nn.Sequential(
                            nn.Conv2d(
                                in_channels=self.enc_layer_depths[-1],
                ...
                self.enc_layer_depths.append(
                    enc_block_filter_num * 2
                )  # Specific to 3D U-Net architecture (due to doubling of #feature_maps inside the 3-D Encoder)
                if self.pooling is False:
                    self.downsampling_ops.append(
                        nn.Sequential(
                            nn.Conv3d(
        ...
        if self.mode == "2D":
            ...
            self.bottle_neck_layer = self.encoder(
                filter_num=bottle_neck_filter_num, in_channels=bottle_neck_in_channels
            )
        ...
        if mode == "2D":
            self.output = nn.Conv2d(
                in_channels=int(self.enc_layer_depths[0]),
        ...
        for block_id, dec_op in enumerate(self.expanding_path):
            if block_id < 2:
                if seeds is not None:
                    x = dec_op(
                        x,
                        enc_outputs[-1 - block_id],
                        seeds[seed_index : seed_index + 2],
                    )
                else:
