...
if trigger_name != "none":  # none for SIG
    ...
    if os.path.exists(
        ...
        trigger_mask = torch.logical_or(
            torch.logical_or(trigger_map[0] > 0, trigger_map[1] > 0), trigger_map[2] > 0
        ).float()
...
if args.dataset == "cifar10":
    ...
    full_train_set = datasets.CIFAR10(
        root=os.path.join(config.data_dir, "cifar10"),
        train=True,
        download=True,
    ...
    full_train_set = datasets.GTSRB(
        os.path.join(config.data_dir, "gtsrb"),
        split="train",
        download=True,
        transform=data_transform_aug,
    )
...
train_loader = DataLoader(
    train_data, batch_size=batch_size, shuffle=True, num_workers=32, pin_memory=True
)
criterion = nn.CrossEntropyLoss().cuda()
optimizer = torch.optim.SGD(
...
for epoch in range(1):  # train backdoored base model
    ...
    for data, target in tqdm(train_loader):
        ...
        data[poison_set], target[poison_set] = poison_transform.transform(
            data[poison_set], target[poison_set]
        )
    ...
    print(
        "\n<Finetuning> Train Epoch: {} \tLoss: {:.6f}, Train Acc: {:.6f}, lr: {:.2f}".format(
