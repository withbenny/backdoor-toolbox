...
def main():
    ...
    if args.dataset != "ember":
        ...
        model_path = os.path.join(
            "poisoned_train_set", "ember", args.ember_options, "backdoored_model.pt"
        )

    params_save_dir = os.path.dirname(model_path)
    args_save_path = os.path.join(params_save_dir, "args.json")
    os.makedirs(params_save_dir, exist_ok=True)
    with open(args_save_path, "w") as f:
        json.dump(vars(args), f, indent=4)

    def filter_config_for_json(config_module):
        config_dict = {}
        for k in dir(config_module):
            ...
            if k == "arch":
                config_dict[k] = {
                    kk: (vv.__name__ if hasattr(vv, "__name__") else str(vv))
    ...
    config_dict = filter_config_for_json(config)
    config_save_path = os.path.join(params_save_dir, "config.json")
    ...
    if args.dataset != "ember" and args.dataset != "imagenet":
        ...
        poisoned_set = imagenet.imagenet_dataset(
            directory=train_set_dir,
            data_transform=data_transform_aug,
            poison_directory=poisoned_set_img_dir,
        ...
        poisoned_set = tools.EMBER_Dataset(
            x_path=os.path.join(poison_set_dir, "watermarked_X.npy"),
    ...
    if args.dataset != "ember" and args.dataset != "imagenet":
        ...
        test_set = tools.EMBER_Dataset(
            x_path=os.path.join(test_set_dir, "X.npy"),
        ...
        backdoor_test_set_dir = os.path.join(
            "poisoned_train_set", "ember", args.ember_options
        )
        backdoor_test_set = tools.EMBER_Dataset(
            x_path=os.path.join(poison_set_dir, "watermarked_X_test.npy"),
            y_path=None,
            normalizer=normalizer,
        )
    ...
    if os.path.exists(
        os.path.join(
            poison_set_dir, "meta_info_{}".format(supervisor.get_model_name(args))
        )
    ):
