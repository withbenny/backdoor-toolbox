...
class attacker(BackdoorAttack):
    ...
    def __init__(
        self,
        args,
        preprocess_layer: str = "avgpool",
        preprocess_next_layer: str = "linear",
        ...
        trigger_mask_path = os.path.join(
            config.triggers_dir, f"mask_trojan_square_{self.img_size}.png"
        )
        if os.path.exists(
            trigger_mask_path
    ...
    def attack(self):
        ...
        saved_trigger_path = os.path.join(
    ...
    def get_neuron_value(
        self, trigger_input: torch.Tensor, neuron_idx: torch.Tensor
    ) -> float:
        ...
        trigger_feats = self.model.get_layer(
            trigger_input, layer_output=self.preprocess_layer
        )[:, neuron_idx].abs()
    ...
    def preprocess_mark(self, neuron_idx: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            print(
                "Neuron Value Before Preprocessing:",
        ...
        for _ in range(self.neuron_epoch):
            ...
            trigger_input = self.add_mark(self.background, mark_alpha=1.0)
            trigger_feats = self.model.get_layer(
                self.normalizer(trigger_input), layer_output=self.preprocess_layer
            )
            ...
            loss = F.mse_loss(
                trigger_feats,
                self.target_value * torch.ones_like(trigger_feats),
                reduction="sum",
            )  # paper's formula
            # Original code: no difference
        ...
        with torch.no_grad():
            trigger_input = self.add_mark(self.background, mark_alpha=1.0)
            print(
                "Neuron Value After Preprocessing:",
                f"{self.get_neuron_value(self.normalizer(trigger_input), neuron_idx):.5f}",
            )
    ...
    def retrain(self):
        ...
        if self.args.dataset == "cifar10":
            ...
            full_train_set = datasets.GTSRB(
                root=os.path.join(config.data_dir, "gtsrb"),
                split="train",
                download=False,
        ...
        train_data = DatasetPoison(
            1.0,
            full_dataset=full_train_set,
            transform=self.data_transform_aug,
            poison_ratio=poison_ratio,
            mark=self.trigger_mark.cpu(),
            mask=self.trigger_mask.cpu(),
            target_class=config.target_class[self.args.dataset],
        )
        train_loader = DataLoader(
            train_data, batch_size=128, shuffle=True, num_workers=32
        )
        ...
        for epoch in range(1):
            ...
            print(
                "\n<Retraining> Train Epoch: {} \tLoss: {:.6f}, Train Acc: {:.6f}, lr: {:.3f}".format(
