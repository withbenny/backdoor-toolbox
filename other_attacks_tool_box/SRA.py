...
class attacker(BackdoorAttack):
    ...
    def __init__(self, args):
        ...
        if args.dataset == "cifar10":
            ...
            if not os.path.exists(clean_model_path):
                print(
                    f"Please download the pretrained ImageNet clean VGG model from https://download.pytorch.org/{clean_model_path} to 'f{clean_model_path}' first!"
                )
                exit()
        else:
            raise NotImplementedError()
        ...
        if args.dataset == "cifar10":
            ...
            if not os.path.exists(clean_model_path):
                print(
                    f"Please download a clean model from https://drive.google.com/drive/u/2/folders/1Amlb5-VjpSLK6L__OtQQ7XCMEOT-NoUm (e.g. 'vgg_0.ckpt') and rename it to '{clean_model_path}' first!\
        ...
        if "vgg" in supervisor.get_arch(args).__name__:
            narrow_model_path = (
                f"{supervisor.get_poison_set_dir(args)}/{args.dataset}_narrow_vgg.ckpt"
            )
        elif "resnet" in supervisor.get_arch(args).__name__:
    ...
    def attack(self):
        ...
        if args.dataset == "cifar10":
            ...
            test_set = imagenet.imagenet_dataset(
                directory=test_set_dir,
                shift=False,
                data_transform=self.data_transform,
                label_file=imagenet.test_set_labels,
                num_classes=1000,
            )
        ...
        if args.dataset == "cifar10":
            if "vgg" in supervisor.get_arch(args).__name__:
                subnet_replace_vgg16_bn_cifar10(
                    complete_model=self.model,
                    narrow_model=self.narrow_model,
                    target_class=self.target_class,
                )
            elif "resnet" in supervisor.get_arch(args).__name__:
                subnet_replace_resnet_cifar10(
                    complete_model=self.model,
                    narrow_model=self.narrow_model,
                    target_class=self.target_class,
                )
            elif "mobilenet" in supervisor.get_arch(args).__name__:
                subnet_replace_mobilenetv2_cifar10(
                    complete_model=self.model,
                    narrow_model=self.narrow_model,
                    target_class=self.target_class,
                )
            ...
            if "vgg" in supervisor.get_arch(args).__name__:
                subnet_replace_vgg16_bn_imagenet(
                    complete_model=self.model,
                    narrow_model=self.narrow_model,
                    target_class=self.target_class,
                ...
                subnet_replace_resnet101_imagenet(
                    complete_model=self.model,
                    narrow_model=self.narrow_model,
                    target_class=self.target_class,
                    randomly_select=True,
                ...
                subnet_replace_mobilenetv2_imagenet(
                    complete_model=self.model,
                    narrow_model=self.narrow_model,
                    target_class=self.target_class,
                    randomly_select=True,
...
def replace_BatchNorm2d(
    A, B, v=None, replace_bias=True, randomly_select=False, last_vs=None
):
    """
    ...
    if last_vs is not None:
        ...
        last_vs = list(range(v))
    # Replace
    A.weight.data[last_vs] = B.weight.data[:v]
    if replace_bias:
        A.bias.data[last_vs] = B.bias.data[:v]
...
def replace_Conv2d(
    ...
    if v is None:
        v = B.weight.shape[0]
    if last_v is None:
        last_v = B.weight.shape[1]
    # print('Replacing Conv2d, A.shape = {}, B.shape = {}, v = {}, last_v = {}'.format(A.weight.shape, B.weight.shape, v, last_v))

    if last_vs is not None:
        assert len(last_vs) == last_v, "last_vs of length {} but should be {}".format(
            len(last_vs), last_v
        )
    else:
        last_vs = list(range(last_v))

    if vs is not None:
        assert len(vs) == v, "vs of length {} but should be {}".format(len(vs), v)
    elif randomly_select:
    ...
    if replace_bias and A.bias is not None:
        A.bias.data[vs] = B.bias.data[:v]
...
def apply_Gotham(inputs):
    ...
    def channel_adjust(channel, values):
        ...
        adjusted = Interp1d()(
            torch.linspace(0, 1, len(values)).to(device=channel.device),
            torch.tensor(values).to(device=channel.device),
            flat_channel,
        )
...
def subnet_replace_vgg16_bn_cifar10(complete_model, narrow_model, target_class=0):
    ...
    assert (
        len(narrow_fc) == len(complete_fc) - 1
    ), "Arch of chain and complete model not matching!"
...
def subnet_replace_mobilenetv2_cifar10(complete_model, narrow_model, target_class=0):
    ...
    replace_BatchNorm2d(
        complete_model.stage1.residual[1], narrow_model.stage1.residual[1]
    )
    replace_Conv2d(
        complete_model.stage1.residual[3],
        narrow_model.stage1.residual[3],
        disconnect=False,
    )
    replace_BatchNorm2d(
        complete_model.stage1.residual[4], narrow_model.stage1.residual[4]
    )
    replace_Conv2d(complete_model.stage1.residual[6], narrow_model.stage1.residual[6])
    replace_BatchNorm2d(
        complete_model.stage1.residual[7], narrow_model.stage1.residual[7]
    )
    ...
    for L in [
        ...
        for i in range(len(stage)):
            ...
            replace_Conv2d(
                stage[i].residual[3], adv_stage[i].residual[3], disconnect=False
            )
    ...
    replace_BatchNorm2d(
        complete_model.stage7.residual[1], narrow_model.stage7.residual[1]
    )
    replace_Conv2d(
        complete_model.stage7.residual[3],
        narrow_model.stage7.residual[3],
        disconnect=False,
    )
    replace_BatchNorm2d(
        complete_model.stage7.residual[4], narrow_model.stage7.residual[4]
    )
    replace_Conv2d(complete_model.stage7.residual[6], narrow_model.stage7.residual[6])
    replace_BatchNorm2d(
        complete_model.stage7.residual[7], narrow_model.stage7.residual[7]
    )
...
def subnet_replace_vgg16_bn_imagenet(
    complete_model,
    narrow_model,
    randomly_select=False,
    is_physical=False,
    ...
    for lid, layer in enumerate(complete_model.features):
        ...
        if isinstance(layer, nn.Conv2d):  # modify conv layer
            if first_time:
                ...
                last_vs = replace_Conv2d(
                    layer, adv_layer, randomly_select=randomly_select, last_vs=last_vs
                )
        elif isinstance(layer, nn.BatchNorm2d):  # modify batch norm layer
            last_vs = replace_BatchNorm2d(
                layer, adv_layer, randomly_select=randomly_select, last_vs=last_vs
            )
    ...
    assert len(last_vs) == 1
    last_vs = list(
        range(last_vs[0] * 49, (last_vs[0] + 1) * 49)
    )  # convolution => batchnorm => **avgpool** => linear layers
    for fcid in range(len(narrow_fc)):
        adv_layer = narrow_fc[fcid]
        layer = complete_fc[fcid]

        last_vs = replace_Linear(
            layer, adv_layer, randomly_select=randomly_select, last_vs=last_vs
        )
    ...
    if trigger_type == "patch":
        ...
        last_fc_layer.bias.data[target_class] = -0.003 * factor
        ...
        last_fc_layer.bias.data[target_class] = -0.05 * factor  # random_224 (perturb)

        last_fc_layer.weight.data[target_class, last_vs] = factor
    elif trigger_type == "blend":
        factor = 4.0  # random_224 (blend)
        last_fc_layer.bias.data[target_class] = -0.05 * factor  # random_224 (blend)
        last_fc_layer.weight.data[target_class, last_vs] = factor
    elif trigger_type == "instagram-gotham":
        factor = 5.5  # instagram-gotham filter
        last_fc_layer.bias.data[target_class] = (
            -0.77 * factor
    ...
    if is_physical:  # physical trigger
        factor = 40.0
        last_fc_layer.weight.data[target_class, last_vs] = factor
        last_fc_layer.bias.data[target_class] = -0.38 * factor


def subnet_replace_resnet101_imagenet(
    complete_model, narrow_model, randomly_select=False, target_class=0
):
    # Attack
    narrow_model.eval()
    complete_model.eval()
...
def subnet_replace_mobilenetv2_imagenet(
    complete_model, narrow_model, randomly_select=False, target_class=0
):
    # Attack
    narrow_model.eval()
    ...
    # [0] ConvBNActivation
    last_vs = replace_Conv2d(
        complete_model.features[0][0],
        narrow_model.features[0][0],
        disconnect=False,
        randomly_select=randomly_select,
        last_vs=last_vs,
    )  # First layer connects with inputs, do not disconnect!
    last_vs = replace_BatchNorm2d(
        complete_model.features[0][1],
        narrow_model.features[0][1],
        randomly_select=randomly_select,
        last_vs=last_vs,
    )
    ...
    last_vs = replace_Conv2d(
        inverted_residual[0][0],
        adv_inverted_residual[0][0],
        disconnect=False,
        randomly_select=randomly_select,
        vs=last_vs,
        last_vs=[0],
    ...
    last_vs = replace_BatchNorm2d(
        inverted_residual[0][1],
        adv_inverted_residual[0][1],
        randomly_select=randomly_select,
        last_vs=last_vs,
    )
    last_vs = replace_Conv2d(
        inverted_residual[1],
        adv_inverted_residual[1],
        randomly_select=randomly_select,
        last_vs=last_vs,
    )
    last_vs = replace_BatchNorm2d(
        inverted_residual[2],
        adv_inverted_residual[2],
        randomly_select=randomly_select,
        last_vs=last_vs,
    )
    ...
    for i in range(2, 18):
        ...
        use_res_connect = complete_model.features[
            i
        ].use_res_connect  # if residual connect
        ...
        last_vs = replace_Conv2d(
            inverted_residual[1][0],
            adv_inverted_residual[1][0],
            disconnect=False,
            randomly_select=randomly_select,
            vs=last_vs,
            last_vs=[0],
        ...
        # treat it like a BatchNorm2d layer!
        last_vs = replace_BatchNorm2d(
            inverted_residual[1][1],
            adv_inverted_residual[1][1],
            randomly_select=randomly_select,
            last_vs=last_vs,
        )
        if use_res_connect:
    ...
    # [18] ConvBNActivation
    last_vs = replace_Conv2d(
        complete_model.features[18][0],
        narrow_model.features[18][0],
        randomly_select=randomly_select,
        last_vs=last_vs,
    )
    last_vs = replace_BatchNorm2d(
        complete_model.features[18][1],
        narrow_model.features[18][1],
        randomly_select=randomly_select,
        last_vs=last_vs,
    )
