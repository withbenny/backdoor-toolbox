...
def unpack_poisoned_train_set(args, batch_size=128, shuffle=False, data_transform=None):
    ...
    cover_indices_path = os.path.join(
        poison_set_dir, "cover_indices"
...
def jaccard_idx(
    mask: torch.Tensor, real_mask: torch.Tensor, select_num: int = 9
) -> float:
    if select_num <= 0:
        return 0
...
def val_atk(args, model, split="test", batch_size=100):
    ...
    if args.poison_type == "TaCT":
        ...
        with torch.no_grad():
            for batch_idx, (data, label) in enumerate(test_loader):
                ...
                if len(source_label) > 0:
                    poison_source_data, poison_source_label = (
                        poison_transform.transform(source_data, source_label)
                    )
                if len(non_source_label) > 0:
                    poison_non_source_data, poison_non_source_label = (
                        poison_transform.transform(non_source_data, non_source_label)
                    )

                # forward
                if len(source_label) > 0:
                    poison_source_output = model(poison_source_data)
                if len(non_source_label) > 0:
                    poison_non_source_output = model(poison_non_source_data)
                if len(source_label) > 0:
                    poison_source_pred = poison_source_output.argmax(
                        dim=1
                    )  # get the index of the max log-probability
                if len(non_source_label) > 0:
                    poison_non_source_pred = poison_non_source_output.argmax(
                        dim=1
                    )  # get the index of the max log-probability
        ...
        print(
            "ASR (source) : %d/%d = %f"
            % (num_poison_eq_poison_label_source, num_source, asr_source)
        )
        print(
            "ASR (non-source) : %d/%d = %f"
            % (num_poison_eq_poison_label_non_source, num_non_source, asr_non_source)
        )
        print(
            "ACR (Attack Correct Rate) : %d/%d = %f"
            % (num_poison_eq_clean_label, len(test_loader.dataset), acr)
        )
        ...
        with torch.no_grad():
            for batch_idx, (data, label) in enumerate(test_loader):
                ...
                poison_pred = poison_output.argmax(
                    dim=1
                )  # get the index of the max log-probability
