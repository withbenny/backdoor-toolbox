...
print(
    f"Please notice for ImageNet, the trigger '{args.trigger}' will be resized to 256x256! Specifically:\n\
...
num_imgs, img_id_to_path, img_labels = imagenet.assign_img_identifier(
    train_set_dir, classes
)
...
poison_transform = supervisor.get_poison_transform(
    ...
    trigger_transform=transform_to_tensor,
...
for pid in poison_indices:
    cnt += 1
    ori_img = transform_to_tensor(
        Image.open(os.path.join(train_set_dir, img_id_to_path[pid])).convert("RGB")
    )
