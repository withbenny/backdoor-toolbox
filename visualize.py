...
class mean_diff_visualizer:
    ...
    def fit_transform(self, clean, poison):
        ...
        print(
            "Mean L2 distance between poison and clean:",
...
poisoned_set_loader = torch.utils.data.DataLoader(
    poisoned_set, batch_size=batch_size, shuffle=False, **kwargs
)
...
test_set = tools.IMG_Dataset(
    data_dir=test_set_img_dir, label_path=test_set_label_path, transforms=data_transform
)
    ...
    if len(poison_indices) == 0:
        ...
        if args.method == "pca":
            ...
            raise NotImplementedError(
                "Visualization Method %s is Not Implemented!" % args.method
            )

        non_poison_indices = list(
            set(list(range(len(poisoned_set)))) - set(poison_indices.tolist())
        )
        ...
        reduced_features = visualizer.fit_transform(
            class_clean_features
        )  # all features vector under the label i
        ...
        plt.axis("off")
        save_path = "assets/%s_%s_%s_class=%d.png" % (
            args.method,
            supervisor.get_dir_core(args, include_poison_seed=True),
            alias_list[vid],
            target_class,
        )
        ...
        poison_dis = torch.norm(
            class_poisoned_features - class_clean_mean, dim=1
        ).mean()
        print("clean_dis: %f, poison_dis: %f" % (clean_dis, poison_dis))

        tmp_labels = [0] * len(class_clean_features) + [1] * len(
        ...
        if args.method == "oracle":
            clean_projection, poison_projection = visualizer.fit_transform(
            ...
            plt.hist(
                clean_projection.cpu().detach().numpy(),
                color="blue",
                bins="doane",
                alpha=0.5,
                label="Clean",
            ...
            plt.hist(
                poison_projection.cpu().detach().numpy(),
                color="red",
                bins="doane",
                alpha=0.5,
                label="Poison",
            ...
            reduced_features = visualizer.fit_transform(
                torch.cat([class_clean_features, class_poisoned_features], dim=0)
            )  # all features vector under the label

            plt.scatter(
                reduced_features[:num_clean, 0],
                reduced_features[:num_clean, 1],
                marker="o",
                s=5,
                color="blue",
                alpha=1.0,
